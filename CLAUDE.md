...
| **ADR-164** | **Platform Trust, Reliability & Capacity Intelligence (PRD 7)** | **F0 (auditoria) + F1 (`PlatformTelemetryContract`/`PlatformTelemetryService`) + F2-processo/runtime (`NodeHostTelemetryProvider`) + F3-SLI HTTP (`HttpMetricsCollector` + middleware) + F4-health de dependências (`DependencyHealthService`) + F5-Operational Health (`OperationalHealthService`) + F6-baseline/anomalia (`PlatformBaselineService` + tabela global `platform_health_snapshots` — motor determinístico que COMEÇA a acumular o baseline; capture horária agregada via Scheduler, seasonality bucket, `insufficient_history` honesto §59, anomalia como hipótese §35; rotas master `platform-baseline`/`platform-anomalies`) + F7-headroom (`CapacityHeadroomService` — folga+zona por recurso HEALTHY→CRITICAL; só o que o Node mede, host/infra `not_available`; trend do baseline F6; rota `capacity-headroom`) FECHADAS. Baseline/forecast só produz resultado útil após dias em produção (§103). Fatia HOST/INFRA da F2 e SLO por jornada (F3.4) pendentes de dados de AMBIENTE. Fecha a lacuna "Delivery Trust": o `ProductionReadinessService` responde "está configurado?", mas não "continuará bem sob carga?". Confirmado: Action Trust (autonomia/policy/executor/rollout/canary/kill-switch/audit) + hooks (`JobQueueService.health` in-process, `SkillOsProviderHealthService` só-IA, `correlationId` de domínio, `Scheduler`, área master "Prontidão de Produção") EXISTEM e serão reusados/estendidos; instrumentação time-series (p50/p95/p99, host/container/DB/fila, requestId/traceId, deploy/commit) é NOVA (sem prom-client/OTel). Guardrails RN-PRC: raw fora do SQLite operacional · recomendação≠execução (V1 nunca redimensiona/migra) · Admin Master only (nunca vaza infra pro tenant) · determinístico antes de LLM · telemetria ausente≠saúde · nunca perde operação crítica · Platform Health Event separado de `business_signals` per-tenant. Análise em `docs/prd/ANALISE-PRD7-vs-CODEBASE-E-INFRA.md`** |
