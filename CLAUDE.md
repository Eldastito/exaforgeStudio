...
| **ADR-163** | **Invisible UX & Zero-Training Experience (PRD 6)** | **F0 (auditoria + matriz de reutilização) FECHADA — restam F1–F12. Simplificação sistêmica de UX + onboarding inteligente + config progressiva; aditivo/reversível sobre a base existente (Fala Tu Home zero-training, Smart Inbox, Context Engine, Entitlement/Blueprint, proatividade). NÃO é redesign nem Home concorrente (§115). Nav vira manifesto DERIVADO de `EntitlementService.overview`+`ContextProjection` (módulos ao 2º nível "Explorar"); "Hoje" ESTENDE `FalaTuHomeService`; inferência ≠ autorização de política material (§27). 3 gaps aditivos: nav declarativa · preferência quiet-hours/limiar · telemetria de UX** |
