...
| **ADR-163** | **Invisible UX & Zero-Training Experience (PRD 6)** | **F0–F11 FECHADAS (11 fatias impl. + 2 docs) — resta só a F12 (redução de legado), adiada até a telemetria da F10 provar substituição (§112). Aditivo/reversível sobre a base existente; NÃO é redesign nem Home/engine concorrente (§115/CA17). Entregues: F2 `NavigationManifestService` (nav por necessidade) · F3 "Hoje" por exceção (ESTENDE `FalaTuHomeService`) · F4 `UxPresentationService` (Decision Card + humanState/humanError) · F5 `AdaptiveOnboardingService` (autodiscovery, nunca inventa) · F6 `InferredSettingsService` (sugere, nunca auto-aplica política — RN-UX-3) · F7 `ZeroTrainingHelpService` (Fala Tu ensine/mostre/faça/onde) · F8 `ExecutionResultsService` (Executando/Resultados) · F9 `ContextualUpgradeService` (upgrade situacional ∩ fora-do-plano, anti-catálogo) · F10 `UxTelemetryService`+`ux_telemetry_events` (LGPD-minimizada, opt-in) · F11 `MobileReadinessService` (fallback garantido, CA1). Rotas em `/api/ux/*` + `/api/entitlements/navigation-manifest`. 5 flags opt-in + 1 tabela nova (F10). Dinheiro role-gated (§73); IA de nav em `docs/ux/ADR-163-F1-information-architecture.md`** |
