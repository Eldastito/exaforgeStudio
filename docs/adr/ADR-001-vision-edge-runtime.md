...
7. **Deploy em produção — RESOLVIDO, ver ADR-008.** A decisão de proxy dentro do `server.ts` evita mudar infraestrutura (DNS, certificado, Traefik/Coolify). Como os dois processos sobem juntos dentro do mesmo container foi decidido e implementado em `docs/adr/ADR-008-process-supervisor.md`: `tini` como PID 1 real (reaping de zumbi/repasse de sinal) + um supervisor Node próprio (`scripts/supervisor.ts`) que sobe `core` e `vision-cloud`, com política de reinício, detecção de processo travado (não só crash) e testes automatizados (`npm run test:supervisor`). A alternativa de dois containers com roteamento na borda foi avaliada e descartada — ver ADR-008 para a comparação completa.
