...
- **Fase 4 — ZappFlow Edge genérico:** generalizar supervisor + SQLite próprio + outbox + auth de máquina; construir o sync Edge↔Cloud que nem o Vision tem ainda (ADR-007). Por ser a fatia mais pesada e horizontal, é decomposta em sub-fases reviewáveis:
  ...
  - **Fase 4b — Runtime do Edge:** processo separado (generalizando `scripts/supervisor.ts` + o modelo `apps/vision-cloud`) com SQLite PRÓPRIO (arquivo distinto, `edge.db`) + outbox (copiando o lease/backoff do `MessageDeliveryService`), dirigindo o sync 4a (pull/push/heartbeat) em loop, com auto-recuperação no reinício.
  - **Fase 4c — Reconciliação bidirecional:** aplicar localmente os eventos puxados por agregado; executar por tipo os comandos recebidos no inbox (`client_commands` status `received`); versionamento/conflitos (optimistic por `updated_at`, evidência nunca sobrescrita — ADR-007 D3); deriva de relógio (D5).
