...
- **Status:** Epic 1 / Fatias B1+B2 implementadas (Business Snapshot V2 + adapters de finanças, vendas, estoque, compras, operação e tarefas; Diretor vê os 6 domínios sob feature-flag). A camada de ação (sinais/decisões/aprovações/Pareto) nas fatias seguintes.
- **Data:** 2026-07
...
`BusinessSnapshotV2Service.build(orgId, period)` devolve `{ organization, period, dataQuality, domains, topPriorities }`. Cada domínio vem de um **adapter** que reusa serviços determinísticos e **falha isolado**: erro em um domínio devolve `{ available:false, error }` sem derrubar o snapshot. **B1** entregou `finance`; **B2** adiciona `sales` (LossMargin/Analytics/QuoteService/RIE), `inventory` (RetailImpactService.stockCapital + estoque baixo/rupturas), `procurement` (requisições abertas + cotações de fornecedor aguardando/respondidas), `retail_ops` (fechamentos pendentes + divergências) e `tasks` (a fazer/fazendo/feito + vencidas + sem responsável). Cada adapter é defensivo (try/catch → `available:false`).
...
`ExecutiveAdvisorService.buildPanorama(orgId)` = `BusinessContextService.build()` + (se `organization_settings.diretor_snapshot_v2=1`) o **bloco V2 por domínio** serializado (finanças, vendas, estoque, compras, operação, tarefas), com instrução explícita: "use EXATAMENTE estes números; NUNCA invente; se faltar, diga que falta". `ask` e `briefing` passam a usar `buildPanorama`. **Flag desligada por padrão** — organizações existentes não mudam de comportamento (rollback = desligar a flag).
...
## Testes
`test:business-snapshot` — `finance` reusa os motores (caixa 5000/fato, a pagar 2000, a receber com vencido 700, previsão como estimativa, DRE com origem); os **demais domínios** (vendas disponível; estoque com capital 1200 e 1 item baixo; 1 requisição de compra aberta; 1 tarefa vencida + 1 sem responsável; operação disponível); org vazia ainda gera snapshot; **flag desligada não altera o panorama do Diretor** (compatível); **ligada injeta o bloco V2 com o caixa REAL** e a instrução anti-invenção; isolamento por org.
