...
- **Status:** Epic 2 / Fatias C1 (Ledger de Sinais), C2a (Ações + Aprovações + Políticas de autonomia), C2b (Outcomes / Impact Ledger unificado), C3 (Pareto / priorização por impacto) e C4 (UI — Plano de Ação no Diretor Executivo IA) implementadas. O executor governado (C5) na fatia seguinte.
- **Data:** 2026-07
...
## Consequências
**Positivas:** cria o contrato único de sinais, o ciclo de ação governado (propor→aprovar→concluir), o Impact Ledger unificado (esperado×realizado, fato≠estimativa), o Pareto que responde "o que atacar primeiro" **e** a tela onde o Diretor vê e age — tudo determinístico, reusando os motores existentes; idempotente e auditável. Aditivo — nada dos fluxos atuais muda.
