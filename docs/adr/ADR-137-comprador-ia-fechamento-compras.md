...
- **Status:** Epic 5 / Fatia E5.1 (cotação aceita → **ordem de compra imutável**) implementada. Recebimento (E5.2), conta a pagar (E5.3) e performance de fornecedor (E5.4) nas fatias seguintes.
- **Data:** 2026-07
...
## Consequências
**Positivas:** o Supply passa a ter um artefato de compra **rastreável e imutável** — base para recebimento, conta a pagar e avaliação de fornecedor. Corrige um contrato quebrado (erro de tipo pré-existente). Aditivo: reusa cotações/requisições existentes; nenhum fluxo atual muda de comportamento (a ordem é criação nova a partir do aceite, que antes só trocava status).
...
## Guardas
- Determinístico (zero-token). Idempotência por `UNIQUE(org, quote_id)`. Snapshot imutável. Isolado por `organization_id`. Sem efeito externo (não envia ao fornecedor).
