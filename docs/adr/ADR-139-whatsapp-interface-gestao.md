...
- **Status:** Epic 3 / Fatias 1 (autenticação + parser + leitura governada por RBAC), 2 (roteamento no processador de mensagens, opt-in) e 3 (**aprovar/dispensar governados** pela política do Epic 2) implementadas. Delegar/adiar/explicar e as preferências de briefing vêm nas fatias seguintes.
- **Data:** 2026-07
...
## Consequências
**Positivas:** o loop **consultar → decidir** fecha pelo WhatsApp (roteado, opt-in) com **autenticação + RBAC + política do Epic 2 + auditoria** — o humano segue no controle e a mesma política do painel vale no canal. Sem risco ao fluxo atual (flag off → Coordenador roda como hoje).

**Trade-offs / escopo:** `delegar`/`adiar`/`explicar` e as preferências de briefing/horários do §14 ficam para as próximas fatias. O roteamento vale no **canal interno** (onde mensagens de equipe/gestão chegam); o gestor mensageando o canal comercial segue o fluxo de cliente.
