...
**Status:** Aceito — **em implementação**. Fatia 34 (esta ADR + travamento de enums/contratos + confirmação com o cliente). **Fatia 35 (Especialidades normalizadas + backfill do texto legado + vínculos N:N profissional↔especialidade) — MERGED.** **Fatia 36 (Episódio de cuidado + transfer/hold/resume/cancel; alta com PIN fica pra Fatia 39) — MERGED.** **Fatia 37 (Aditivos em appointments + gate EPISODE_PROFESSIONAL_MISMATCH + assistente Adicionar Especialidade atômico) — MERGED.** **Fatia 38 (Ciclos de sessões renováveis, saldo derivado por query — RN-004, renewalQueue, hook de transição on completion, ciclo inicial no addSpecialtyForPatient) — MERGED.** **Fatia 39 (Alta explícita com PIN obrigatório + reopen com PIN — RN-007; gate EPISODE_DISCHARGED em createAppointment; appointments futuros NÃO cancelados na alta) — MERGED.** **Fatia 40 (Métricas de jornada + fila operacional + counts pra badge — RF-100 §5; fecha a Fase 2) — MERGED.** Fase 2 CONCLUÍDA. **Fatia 41 (Sessões de agenda compartilhadas — D6 primeira classe: DB + service + rotas + transação atômica AC-012; refactor do findConflicts fica pra Fatia 42) — MERGED.** **Fatia 42 (Refactor findConflicts pra RN-006 — grupo de N = 1 ocupação; checkRoomCapacity + ROOM_CAPACITY_EXCEEDED; remove force=true temporário da Fatia 41) — MERGED.** **Fatia 43 (Ocupação por sessão — RN-006 métrica: grupo de 5 = 1 ocupação; groupInfoForOwnAppointment no portal do paciente — RN-013 §3: agregados sem nomes de outros; fecha a Fase 3) — MERGED.** Fase 3 CONCLUÍDA. **Fatia 44 (Guia da recepção polimorfa D7 — DB clinical_guides + 3 tipos TISS/referral/medical_order + numeração série própria por tipo + snapshot canônico + document_hash imutável Fase 29) — MERGED.** **Fatia 45 (Guia PDF polimorfo por tipo + envio HMAC padrão Fase K/18/33 + ClinicGuideDeliveryService + LGPD dados_sensiveis + comunicacoes + rota pública com nosniff + toIdentifier mascarado Fase 32) — MERGED.** **Fatia 46 (Integração guide↔cycle bidirecional + gate RN-005 §8 pending_authorization + linkGuide + hook transitionOnGuideIssued via import dinâmico + config org clinic_cycle_requires_guide + renewalQueue inclui pending_authorization; fecha a Fase 4) — MERGED.** Fase 4 CONCLUÍDA. **Fatia 47 (IA operacional — Fase 5 §F47: ClinicScheduleSessionService.availability determinístico + ClinicRenewalTaskService varre renewalQueue → publica BusinessSignal domain='clinic' idempotente + guardrails RN-014 documentados — IA nunca sugere outro profissional, nunca inventa TUSS/carteirinha/autorização, nunca renova/dá alta/emite guia sem humano) — MERGED.** **Fatia 48 (Fase 5 §F48: ClinicGuideService.draft — rascunho pré-preenchido em memória a partir de patient_profiles + procedure_authorization_requests + guia anterior + último atestado; campos sem fonte viram {missing:true, reason} — IA nunca fabrica TUSS/carteirinha/authorizationNumber/validUntil; referralReason nunca é herdado; medical_order.items sempre missing; rota POST /guides/draft; fecha a Fase 5) — MERGED.** Fase 5 CONCLUÍDA. **ADR-145 FECHADO — as 5 dores estruturais da clínica multiespecialidade (multiespecialidade sem recadastro, ciclos renováveis com saldo derivado, grupo primeira classe, alta com PIN, guia da recepção 3 tipos) e a IA operacional (sugestão de horário + sinais de renovação + rascunho de guia sem invenção) estão em produção.**
