...
- **Status:** EM PROGRESSO. **Fase 1 (F1.1–F1.3)** implementada: grimoire (`docs/grimoire/copy/` + 4 rubricas), `GrimoireService.load` (roteamento just-in-time, compilado no build), camada por-org (`brand_voice_context` + `promptForOrg` gated). **Fase 1 e Fase 2 FECHADAS.** O ciclo completo **grimoire → copy → medição → aprendizado** está em produção: rubricas roteadas just-in-time (F1.1–F1.2) + camada por-org (F1.3) + copy de cobrança calibrada com A/B e soft/hard decline (F2.1–F2.2) + medição do A/B em `business_signals` (F2.3) + **pós-mortem que reinjeta a lição no grimoire (F1.4)**. **Fase 4 FECHADA (F4.1–F4.2)** — `ChurnRiskDetectorService` (health-score 0–100 do cliente por query: fatura vencida + silêncio + ticket frio) publica `churn_risk_high` em `business_signals` (opt-in, sweep, advisory RN-014) + **card na aba Operações** que mostra score/explicabilidade e deixa o humano decidir (acknowledge/dismiss). **Fase 5 FECHADA (F5.1–F5.3)** — save offer no cancel/refund do FalaTu: captura do motivo + mapa motivo→degrau do ladder (`FalatuSaveOfferService` + tabela `falatu_cancellation_intents` + rota `POST /save-offer/intent`); **aceite governado** (`acceptOffer` + `POST /save-offer/accept`: calcula o alvo do downgrade pelo catálogo, marca `retained` e publica handoff `save_offer_accepted` pro operador finalizar em Cobrança — G-153-3 / RN-155 §2, **sem** mexer em billing/ASAAS); e **medição de retenção** (reembolso fecha o outcome em `refunded` best-effort; `retentionMetrics`/`retentionSummary` derivam retidos-vs-reembolsados por query + `byOffer`; rota admin `GET /api/admin/falatu/save-offer-retention`). Guardrail duro nas três: a garantia de 7 dias (CDC Art. 49) **nunca** é bloqueada (todo retorno carrega a elegibilidade do reembolso). **Demais fases (F3 Recuperação, F6 Referrals) aguardam priorização do dono da plataforma.**
- **Data:** 2026-08-08
...
- **2026-08-08** — **F5.3 implementada (Fase 5 FECHADA)**: medição de retenção (retidos vs reembolsados), **derivada por query** sobre `falatu_cancellation_intents.outcome` (RN-004 — nenhum contador mutável, nenhuma tabela nova, nenhum aditivo em `db.ts`). (1) **Fecha o loop do outcome**: `FalatuRefundService.requestRefund`, depois de estornar+cancelar, marca a intenção `pending` como `refunded` via **import dinâmico** (convenção nº 11, quebra o ciclo) em **best-effort** (convenção nº 7 — o try/catch garante que a medição NUNCA derruba o reembolso money-critical). (2) **`retentionMetrics(orgId)`** (por org, isolado) e **`retentionSummary()`** (cross-org, só master admin) agregam por query: counts por `outcome`, `retentionRatePct = retained/resolved` (**null** quando não há caso resolvido — honestidade sobre o que o número mede, padrão 5, não força 0%), e breakdown `byOffer`/`byReason` (qual degrau do ladder retém melhor — insumo pra afinar a oferta). (3) Rota `GET /api/admin/falatu/save-offer-retention` (gateada pelo `requireMasterAdmin` do mount). **Sem `business_signal` aqui** (diferente da F2.3): retenção é evento de ciclo de vida one-shot lido do próprio `outcome`, não alerta operacional recorrente por org — publicar taxa per-org seria ruído. Teste `test:falatu-save-offer` ampliado (62 checagens): reembolso→`refunded` (e best-effort sem intenção), `retentionMetrics` (counts/rate/byOffer/byReason), rate `null` honesto, `retentionSummary` cross-org + isolamento. `tsc` + regressão `test:falatu-refund` verdes. **Fase 5 completa: captura (F5.1) → aceite governado (F5.2) → medição (F5.3), com a garantia de 7 dias intocada em todas.**
- **2026-08-08** — **F5.2 implementada**: aceitar a save offer **ligando no entitlement (ADR-153) sem autonomia de cobrança**. `FalatuSaveOfferService.acceptOffer` segue o **G-153-3** (na ADR-153, aceitar recomendação nunca executa a mudança — só registra e manda o humano finalizar em Cobrança) e o **RN-155 §2** (a IA não muda billing sozinha): calcula o **alvo do downgrade** a partir do catálogo (`downgradeTargetFor` deriva o tier imediatamente abaixo de `FALATU_PLAN_IDS` — familia→pro→solo, ordem de preço), marca a intenção como **`retained`** (RN-004: outcome é o dado, F5.3 deriva por query) e publica um sinal de handoff `save_offer_accepted` em `business_signals` (`domain=churn`, `severity=attention` p/ downgrade/pause, `info` p/ roadmap/support; `basis=fact`, `impact`=MRR retido; convenção nº 12) pro operador aplicar a pausa/downgrade em Cobrança. **NÃO chama `setPlan`/`setBillingStatus`/ASAAS** — o teste prova que billing e plano ficam intactos após aceitar, e que `eligibility` segue elegível (garantia de 7 dias nunca tocada). `outro`→`none` não é aceitável (`no_offer`) e a intenção segue `pending` (vai pro reembolso). Rota `POST /save-offer/accept` (sem pending/oferta → 400). Teste `test:falatu-save-offer` ampliado (48 checagens): `downgradeTargetFor` por tier, accept governado (retido + handoff + billing/plano intactos + garantia intacta + sinal), pause/roadmap/support/none, isolamento, rota. `tsc` + regressão `test:falatu-refund` + `test:churn-risk-detector` verdes. Resta a **F5.3** (medição de retenção: retidos vs reembolsados).
- **2026-08-08** — **F5.1 implementada (Fase 5 em progresso)**: save offer no cancel/refund do FalaTu — captura do motivo + mapa motivo→degrau do ladder (destila a rubrica `compose/save-offer-ladder.md`). Nova tabela `falatu_cancellation_intents` (aditivo no fim do `db.ts`, convenção nº 2): registra a INTENÇÃO com o motivo capturado, o degrau ofertado e o `outcome` (`pending` → `retained`|`refunded`|`cancelled`; insumo da medição F5.3). `src/server/FalatuSaveOfferService.ts`: `offerForReason` (puro) mapeia `preco→downgrade`, `pouco_uso→pause`, `faltou_feature→roadmap`, `problema_tecnico→support`, `outro→none` (oferece **um** degrau, não todos); `captureIntent` faz **upsert de UMA intenção `pending` por org** (dedupe) e **SEMPRE devolve a elegibilidade do reembolso** (`FalatuRefundService.checkEligibility`) + `refundNote`. **Guardrail money-critical (RN-155 §5 / ADR-154 RN-E): a garantia de 7 dias (CDC Art. 49) NUNCA é bloqueada pela oferta** — a save offer é opt-out, recusar leva DIRETO ao reembolso; por isso todo retorno (e a rota `POST /save-offer/intent`) carrega a `eligibility`, elegível ou não. Este serviço só **sugere** — não pausa, não dá desconto, não estorna (a EXECUÇÃO do degrau ligando no entitlement ADR-153 é a F5.2; a medição de retenção aceitos-vs-reembolsados é a F5.3). Teste `scripts/test-falatu-save-offer.ts` (`npm run test:falatu-save-offer`, 29 checagens): mapa motivo→degrau, `captureIntent` com eligibility sempre presente (elegível e não-elegível), dedupe/upsert, isolamento multi-tenant, `resolve`, rota (motivo válido → 200 com eligibility; inválido → 400). `tsc` + regressão `test:falatu-refund` (garantia intacta) verdes. Próximas: **F5.2** (ladder ligando no entitlement) e **F5.3** (medição de retenção).

## Histórico (Fase 4)

- **2026-08-08** — **F4.1 implementada**: `src/server/ChurnRiskDetectorService.ts` — porta o health-score `churn-prevention` (rubrica `intake/churn-risk-scoring.md`) pra um detector de risco de churn do **cliente (contato)**. Score 0–100 **derivado por query** (RN-004): fatura vencida (`receivables` open+past due, peso maior), silêncio no canal (dias desde a última msg do contato, proxy de "uso caindo"), ticket aberto frio. `detect()` puro; `publish()` publica `churn_risk_high` em `business_signals` (domain `churn`, convenção nº 12) só pra score ≥ 70 ("alto"), com **explicabilidade rica** (breakdown por fator + `factors` em PT-BR, molde PlanFitDetector F7.2) e **sweep** (contato que sai do risco → sinal resolvido). **Opt-in por org** (`churn_detector_enabled`, aditivo default 0 ⇒ zero mudança em prod). **Guardrail RN-014** na evidência: sugere retenção, humano decide — não cancela, não dá desconto, não renova. Wired no `Scheduler.churnRiskPass` (após o `planFitPass`). Teste `scripts/test-churn-risk-detector.ts` (`npm run test:churn-risk-detector`, 20 checagens): score/severidade/breakdown/factors, threshold 70, publish + evidência RN-014, sweep, opt-in, isolamento. `tsc` + `vite build` verdes. Próxima: **F4.2** (card na aba Operações consome a evidência).
- **2026-08-08** — **F4.2 implementada (Fase 4 FECHADA)**: card "Clientes em risco de churn" na aba Operações (`ExecutiveView` / `OperacoesTab`). Novo endpoint `GET /api/runtime/operations/churn` (sinais `churn_risk_high` abertos, via `BusinessSignalService.list`) + `POST /operations/churn/:id/:action` (acknowledge|dismiss). O card mostra score/faixa, o valor em aberto, os `factors` (explicabilidade F4.1) e a nota RN-014; os botões **"Vou cuidar" (acknowledge)** / **"Não é risco" (dismiss)** materializam o "humano decide" — a UI **nunca age** (não cobra, não dá desconto). Só aparece se há sinal aberto. Fatia de UI: gate `npm run lint` (tsc --noEmit) + `vite build` verdes; endpoint fino sobre serviços já testados (padrão das fatias de UI anteriores). Regressão `test:churn-risk-detector` (20) verde.
...
- **2026-08-08** — **F2.2 implementada** (retry diferenciado soft/hard decline): `CollectionCadenceService.classifyDecline(dueDate, hardDeclineDays)` (público/puro) classifica pelo atraso — `>= collection_hard_decline_days` (novo aditivo, default 7) ⇒ **hard** (via provavelmente expirada), senão **soft**. `CollectionCopy.firm`/`notice` ganham 3º param `decline` (default `soft`): a variante `control` **ignora** (byte-idêntica ⇒ zero mudança), a `calibrated` ramifica — soft re-nudge do PIX enviado; **hard oferece 2ª via atualizada** ("me responde que te mando uma atualizada" — honesto: o resend existe no reply router F4b.3, sem prometer o que não há). Ambas mantêm valor+vencimento e o aviso final mantém o informativo CDC. `sendAttempt` classifica, passa pra copy e grava `collection_followup_attempts.decline_type` (novo aditivo) + audit. Teste `scripts/test-collection-decline.ts` (`npm run test:collection-decline`, 29 checagens). Regressão verde: `collection-copy` (22), `cobranca-cadencia-multitentativa` (38), `pilot-runtime` (55), `piloto-cobranca` (38), `cobranca-promise-recheck` (33). Só copy; canal/timing/guardas da cadência intactos.
- **2026-08-08** — **F2.3 implementada (Fase 2 FECHADA)**: `src/server/CollectionAbMeasurementService.ts` — atribuição **nível-ação** por variante (a cobrança recuperada = `decision_action status='done'` + `result_amount`, régua da fundação F3.1) + breakdown de decline nível-tentativa; elege vencedor só com amostra ≥ 5 (anti-ruído). Derivado por query (RN-004), sem contador mutável. Publica o KPI `collection_ab_result` em `business_signals` (`severity=info`, `dedupeKey` estável por org ⇒ upsert = KPI vivo; convenção nº 12). Wired no `Scheduler.collectionCadencePass` (após o `tickAll`, best-effort). Teste `scripts/test-collection-ab-measurement.ts` (`npm run test:collection-ab-measurement`, 21 checagens): agregação por variante (sent/recovered/revenueCents/rate), breakdown de decline, vencedor com amostra mínima, publish + upsert, isolamento multi-tenant. `tsc` + `vite build` + regressão (collection-copy 22, collection-decline 29, cadencia-multitentativa 38, pilot-runtime 55) verdes. **Fecha o ciclo grimoire → copy → medição.** Resta só a **F1.4** (pós-mortem: A/B fraco → `Lição` na rubrica), agora desbloqueada.
...
- **2026-08-08** — **Fase 1 redesenhada** para adotar o **padrão 4 (grimoire / progressive-disclosure)** do `docs/patterns/agentic-pipeline-lessons.md` (estudo do img2threejs): de "pasta flat de playbooks + blob de contexto" para um **grimoire roteado por estágio de decisão**, carregado just-in-time por um `GrimoireService`, com cada rubrica em formato de contrato (`Quando/Deve/Nunca/Exemplos/Lições`), 2 camadas (global versionada + `brand_voice_context` por org) e acúmulo de lições pós-mortem. Fatias reescritas: F1.1 esqueleto+rubricas, F1.2 loader+roteamento, F1.3 camada por-org, F1.4 canal de pós-mortem.
- **2026-08-08** — **F1.1 implementada**: `docs/grimoire/copy/` com taxonomia por estágio (`intake/compose/guardrails/review/glossary`), `_TEMPLATE.md` (contrato), `INDEX.json` (roteamento `(módulo,estágio)→rubrica`), as 4 rubricas-núcleo (`dunning-cadence`, `save-offer-ladder`, `sequence-timing`, `churn-risk-scoring`) traduzidas PT-BR com atribuição MIT + 3 rubricas internas de suporte (guardrails LGPD/WhatsApp, checklist pré-envio, tom de voz). Validador estrutural determinístico `scripts/test-grimoire-copy.ts` (`npm run test:grimoire-copy`, 156 checagens) — no espírito do padrão ("scripts enforçam estrutura"). Sem runtime ainda (F1.2).
- **2026-08-08** — **F1.2 implementada**: `src/server/GrimoireService.ts` com `load(orgId, module, stage)` — roteia pelo `INDEX` e injeta **só** a(s) rubrica(s) roteada(s) como bloco `<rubrica>` (progressive disclosure), com `orgId` tenant-first (camada por-org é F1.3) e isolamento por módulo. O grimoire é **compilado num módulo TS embarcável** (`src/server/grimoire/compiled.ts`, gerado por `scripts/build-grimoire.ts` / `npm run grimoire:build`) — sem `fs` em runtime no bundle esbuild. Teste `scripts/test-grimoire-service.ts` (`npm run test:grimoire-service`, 23 checagens) prova isolamento por módulo, progressive-disclosure (prompt < dump), graceful pra desconhecido e **freshness** do compilado. Nenhum redator consome ainda — wiring é F2/F3.
- **2026-08-08** — **F1.3 implementada**: camada por-org do grimoire. Aditivos em `organization_settings` (`brand_voice_context TEXT`, `brand_voice_enabled INTEGER DEFAULT 0`). `GrimoireService.getBrandVoice`/`setBrandVoice`/`promptForOrg` (db via import dinâmico, convenção nº 11): `promptForOrg` é a **injeção combinada gated** — flag off (default de toda org) ⇒ retorna `""` (zero mudança em prod); flag on ⇒ rubrica global roteada + `<contexto_marca>` da org. Teste `scripts/test-grimoire-brand-voice.ts` (`npm run test:grimoire-brand-voice`, 16 checagens) prova gating pela flag, injeção combinada e **isolamento multi-tenant** (contexto de uma org nunca vaza pra outra). **Decisão de escopo:** o *mecanismo* de injeção (`promptForOrg`) está pronto e testado, mas a **adoção por redator concreto fica pra F2/F3** — os redatores de copy-ao-cliente hoje são heterogêneos (Collection é template-based; o redator LLM do FalaTu é interpretação de captura, não copy), e é em F2/F3 (tune-up da copy) que cada um passa a chamar `promptForOrg`. Nada consome ainda ⇒ zero mudança em prod. UI de edição do brand voice é a F3 (FalaTuSettingsView).
