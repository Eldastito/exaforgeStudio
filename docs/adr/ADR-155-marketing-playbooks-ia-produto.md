...
- **Status:** EM PROGRESSO — Fase 1 com o padrão grimoire (padrão 4 do `docs/patterns/agentic-pipeline-lessons.md`). **F1.1 + F1.2 implementadas**: esqueleto `docs/grimoire/copy/` + 4 rubricas-núcleo + `GrimoireService.load(orgId, module, stage)` (roteamento just-in-time, grimoire compilado no build via `npm run grimoire:build`, isolamento por módulo). Próxima: **F1.3** (`brand_voice_context` por org + injeção combinada no prompt dos redatores). Demais fases aguardam priorização do dono da plataforma.
- **Data:** 2026-08-08
