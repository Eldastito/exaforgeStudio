...
- **Status:** **FECHADO** (2026-08-09) — DI-5.1..DI-5.5 entregues (PRs #872 em diante), todas as fatias com CI verde. Evolui a ADR-156 (External Intelligence de vertical compartilhada) do modelo **manual** (o admin cola) para um modelo **automático curado** (o ZapFlow pesquisa via `LlmResearchProvider`, o `ResearchCuratorService` cura e publica sozinho, base longitudinal com delta em `vertical_intelligence_history`, agenda por nicho no Scheduler com teto/nichos/intervalo, e UI de automação + tendência no `NicheIntelligenceView`), preservando **todos** os guardrails de anonimização/isolamento da ADR-156.
- **Data:** 2026-08-09
