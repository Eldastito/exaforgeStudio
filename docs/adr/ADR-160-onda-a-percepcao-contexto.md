...
- **Status:** **EM ANDAMENTO** — abre a **Onda A** do programa ZapFlow Execution Intelligence, sobre a base já consolidada pela Onda 0 (ADR-158 espinha única + ADR-159 governança, ambas fechadas em produção). **F1 (leitura transversal de atenção) ENTREGUE**; **F2 (snapshot como leitura default) ENTREGUE**; **F3 (convergir Context+V2) ENTREGUE**; **F4 (metas + distância à meta) ENTREGUE**; **F5 (Fala Tu → porta I/O: bridge de tarefas) ENTREGUE** — 1ª fatia do thread de maior risco; F6+ (eventos→Appointment, listas, roteamento WhatsApp, dedup de RAG) seguem.
- **Data:** 2026-08-10
...
> **Não abrir** eventos/listas/roteamento antes do bridge de tarefas estar estável em produção — a convergência avança por domínio, medindo regressão a cada passo.
...
| F6+ | D5 — porta I/O: eventos→Appointment, listas, roteamento WhatsApp p/ choke-point, dedup de RAG | planejada |
