...
**Estado:** Em implementação — **F0–F10 FECHADAS (+ recomendação advisória explicável). O baseline/forecast SÓ produz resultado útil após dias acumulando em produção (§103); a fatia HOST/INFRA da F2 e o SLO por jornada (F3.4) dependem de dados de AMBIENTE**
