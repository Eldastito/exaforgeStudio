...
**Estado:** Em implementação — **F0 + F1 + F2(processo/runtime) + F3(SLI HTTP) + F4(health de dependências) FECHADAS; fatia HOST/INFRA da F2, o SLO por jornada (F3.4), F5 e a F6+ (baseline/anomalia/headroom/forecast) dependem de dados de AMBIENTE / baseline real**
...
| F5 | Operational Health (estende Production Readiness) | ESTENDER |
