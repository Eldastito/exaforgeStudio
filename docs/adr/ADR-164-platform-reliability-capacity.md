...
**Estado:** Em implementação — **F0–F13 FECHADAS (+ Capacity Envelope; números reais dependem do ambiente). O baseline/forecast SÓ produz resultado útil após dias acumulando em produção (§103); a fatia HOST/INFRA da F2 e o SLO por jornada (F3.4) dependem de dados de AMBIENTE**
...
| **F6** | **Baselines & Anomaly detection (`PlatformBaselineService`) FECHADA — motor que COMEÇA a acumular o baseline** | CRIAR |
| **F7** | **Capacity Headroom por recurso + zonas (`CapacityHeadroomService`) FECHADA (motor; output útil após baseline)** | CRIAR |
| **F8** | **Capacity Forecast (trend+seasonality+confidence) via `CapacityForecastService` FECHADA — output útil após baseline** | CRIAR |
| **F9** | **Root-cause correlation (infra×app×DB×fila×provider×deploy) via `PlatformRootCauseService` FECHADA — hipóteses, não veredito** | CRIAR + COMPOR |
| **F10** | **Recommendation Engine (advisório, explicável) via `CapacityRecommendationService` FECHADA — nunca executa** | CRIAR |
| **F11** | **Reliability-aware execution (Protection Mode, sem Policy Engine) via `PlatformProtectionModeService` FECHADA — SHADOW por padrão** | CRIAR estado + REUTILIZAR Runtime |
| **F12** | **Master Alerts (Platform Health Events, anti-spam) via `PlatformAlertService` FECHADA** | REUTILIZAR canais |
| **F13** | **Load tests & Capacity Envelope (fora de produção) via `CapacityEnvelopeService` + harness FECHADA — números reais dependem do ambiente** | CRIAR controlado |
...
- **F13 — FECHADA (Capacity Envelope + harness fora de produção).** `src/server/CapacityEnvelopeService.ts` + harness `scripts/loadtest-capacity.ts` (`loadtest:capacity`, **fora do CI** — sem prefixo `test:`). O Envelope é o LIMITE SEGURO de operação ("aguenta ~X req/s antes do p95 furar o SLO"), DERIVADO de teste de carga rodado FORA de produção (§104-§108, CA21). `deriveEnvelope(samples, slo)` é função PURA que acha o "joelho" (maior rps com p95 ≤ SLO e erro ≤ limite): piso já furando → `safeRps:0/over_slo_at_min_load`; sem violação no range → `no_knee_within_tested_range` (aguenta AO MENOS o testado). Envelope VERSIONADO (CA21), persistido em `platform_settings` (GLOBAL). **Guardrail central (§59/§103):** sem teste de carga real, `current()` responde `established:false/awaiting_load_test` — **não inventa limite**; o GitHub prova o motor, o número real vem do ambiente (harness aponta pra STAGING, revisão humana antes de `store`). `headroomVs(currentRps)` dá folga/utilização vs envelope. Rotas master `GET/POST /api/admin/capacity-envelope`. `test:capacity-envelope` (10 checks). **0 tabelas novas, 0 flags novas** (envelope em `platform_settings`), 0 breaking changes.
- **F12 — FECHADA (Master Alerts com anti-spam).** `src/server/PlatformAlertService.ts` + tabela **global** `platform_health_events` — o "Platform Health Event separado de `business_signals` per-tenant" que o RN-PRC exige: saúde de INFRA é do Admin Master, não de um tenant, então **não** usa `business_signals` (per-org). **Anti-spam (CA17 — avisar antes do crítico, sem afogar):** um evento ABERTO por `dedupe_key` (UNIQUE); reincidência bumpa `occurrences`/`last_seen_at` em vez de duplicar, e a NOTIFICAÇÃO só reflora depois da janela `NOTIFY_WINDOW_MS` (6h, via `notified_at`) — problema persistente não vira enxurrada. `refresh(recomendações)` compõe com a F10: recomendação de prioridade ALTA vira/atualiza evento; recomendação que sumiu → **auto-resolve** (recuperou). **Guardrails:** RN-PRC-4/§46 — GLOBAL, nunca vaza pro tenant; RN-PRC-3 — grava só o EVENTO (incidente), nunca raw time-series; entrega nos canais (Fala Tu/Push/in-app/e-mail) é best-effort (convenção 7, nunca quebra o caller). Scheduler faz o `refresh` horário quando a telemetria está ligada (proativo). Determinístico (`now` injetável). Rotas master `GET /api/admin/platform-alerts` + `POST /api/admin/platform-alerts/refresh`. `test:platform-alerts` (13 checks). Regressão `capacity-recommendation` PASS. **1 tabela nova (global, aditiva no fim do `db.ts`), 0 flags novas**, 0 breaking changes.
- **F11 — FECHADA (Protection Mode shadow-first).** `src/server/PlatformProtectionModeService.ts` — DERIVA uma postura de confiabilidade da plataforma (NORMAL→CAUTIOUS→PROTECTED) da saúde operacional (F5) + headroom (F7), que o Runtime PODE consultar pra ficar conservador sob estresse (§93-§98). **NÃO é Policy Engine novo (D1/CA20):** não decide autorização nem substitui `ApprovalPolicyService`/`CommandExecutorService` — só expõe estado derivado + um flag. **SHADOW-FIRST (§102):** o flag `platform_protection_enforce_enabled` (em `platform_settings`, GLOBAL) nasce DESLIGADO; enquanto desligado, `active:false` — a postura é reportada mas NÃO altera comportamento; ligar é decisão humana explícita (`POST /protection-mode/enforce`). **CA23/RN-PRC-7 — o Guard NUNCA sacrifica operação crítica:** mesmo em PROTECTED só trabalho ADIÁVEL de plataforma (captura de baseline pesada/digests/recomputações em lote) é candidato a adiar; `neverDefers` (operação de cliente, execução confirmada, cobrança, alertas críticos) é explícito no payload e nunca intersecta `wouldDefer`. Sem LLM (§56). Entradas injetáveis → determinismo. Rotas master `GET /api/admin/protection-mode` + `POST /api/admin/protection-mode/enforce`. `test:platform-protection` (13 checks). Regressão `operational-health`/`capacity-headroom` PASS. **0 tabelas novas, 1 flag global** (`platform_settings`, não per-tenant), 0 breaking changes.
- **F10 — FECHADA (recomendação advisória e explicável).** `src/server/CapacityRecommendationService.ts` — junta as três leituras já existentes (headroom AGORA da F7 + forecast FUTURO da F8 + causa provável da F9) e produz recomendações que o operador **lê e decide**, cada uma **citando a evidência** que a gerou (§77/CA15 — explicável). **Guardrails (o coração do PRD 7):** D6/CA16/RN-PRC — **V1 NUNCA executa** (não redimensiona, não migra, não compra): toda rec é `requiresHuman:true`/`autoExecuted:false`/`basis:"advisory"`, com `disclaimer` fixo; RN-PRC-1 — nunca recomenda de pico único (só zona SUSTENTADA da F7 ou tendência com confiança ≥ média da F8; forecast de confiança **baixa é ignorado**); RN-PRC-6/§59 — sem sinal → `all_clear` (não inventa urgência); §35 — recomendação sobre causa é hipótese ("investigar antes de agir"), confiança conservadora. Ordena por prioridade (alta/média/baixa derivada de zona/proximidade). Entradas injetáveis (headroom/forecast/rootCause) → determinismo total. Sem LLM (§56). Rota master `GET /api/admin/capacity-recommendations`. `test:capacity-recommendation` (12 checks). Regressão `capacity-headroom`/`capacity-forecast`/`platform-rootcause` PASS. **0 tabelas novas, 0 flags novas**, 0 breaking changes.
- **F9 — FECHADA (correlação de causa provável).** `src/server/PlatformRootCauseService.ts` — quando a F6 aponta anomalias em várias camadas ao mesmo tempo, COMPÕE (não recalcula: reusa `PlatformBaselineService.anomalies`) e aplica um conjunto DETERMINÍSTICO de regras sintoma→causa-provável (`db_contention`=p95+banco lento; `cpu_saturation`=p95+load; `event_loop_blocking`=p95+event-loop lag; `queue_backpressure`=fila+erro; `dependency_errors`=erro; `memory_growth`=rss subindo), produzindo HIPÓTESES ranqueadas (§86-§92, CA7). **Guardrails:** §35 — resultado é SEMPRE hipótese (`basis:"correlation"`) com nota "correlação, não causa comprovada", NUNCA veredito; RN-PRC-6 — anomalia sem regra vira `unexplainedDeviations` (não força causa) e sem anomalia → sem hipótese (não inventa); CA8 — `deployCorrelation` declarado `not_available/no_deploy_telemetry` enquanto não houver telemetria de deploy/commit (a F0 provou que não existe); sem LLM (§56/§57); GLOBAL/Admin Master (§46). Determinístico (ordem de regras fixa, `now` injetável). Rota master `GET /api/admin/platform-root-cause`. `test:platform-rootcause` (8 checks). Regressão `platform-baseline`/`capacity-forecast` PASS. **0 tabelas novas, 0 flags novas**, 0 breaking changes.
- **F8 — FECHADA (motor de forecast).** `src/server/CapacityForecastService.ts` — projeta a trajetória de uma métrica a partir do histórico **agregado** da F6 (`platform_health_snapshots`, nunca raw — RN-PRC-3) e estima **quando cruza o limiar crítico**, sempre com **confiança** explícita (§80-§85, CA10/CA11/CA22). Método determinístico (sem LLM §56): agrega por DIA (bucket SP) pra **remover a sazonalidade intradiária** (§33) antes de medir a tendência → ajuste linear (mínimos quadrados) → `slopePerDay` + `r2` → confiança alta/média/baixa; projeta `horizonDays` à frente e calcula `daysToTarget` **só quando a inclinação aponta para o alvo** (senão `not_approaching`/`already_at_or_above`/`flat_or_declining`). **Guardrails:** §59/§103/RN-PRC-9 — menos de `MIN_DAYS=5` dias distintos → `insufficient_history` (**não inventa**); RN-PRC-1 — série de vários dias, nunca pico único; §35 — resultado é HIPÓTESE (`basis:"trend"`), nunca veredito; RN-PRC-4/§46 — GLOBAL/Admin Master. `forecastCapacity()` compõe as métricas de capacidade conhecidas (mem/load/db/fila/p95) rumo ao crítico e aponta o **primeiro gargalo** (menor `daysToTarget`). Rota master `GET /api/admin/capacity-forecast` (`?metric=`/`?days=`/`?horizon=`; sem metric → visão de capacidade). `test:capacity-forecast` (13 checks). Regressão `capacity-headroom`/`platform-baseline` PASS. **0 tabelas novas, 0 flags novas** (reusa `platform_health_snapshots`+`platform_telemetry_enabled`), 0 breaking changes.
- **F7 — FECHADA (motor de headroom).** `src/server/CapacityHeadroomService.ts` — headroom = folga até a zona insegura **por recurso** (§26 — não há headroom universal); zonas HEALTHY→OBSERVE→PLAN→ACT→CRITICAL (§27). Calcula só o que o processo já observa (memória do host em %, load por core — via `NodeHostTelemetryProvider`); recurso que exige o provider de host (disco/rede/swap/limites de container) → `NOT_AVAILABLE`/`requires_host_provider` (RN-PRC-6, honesto). `trend` vem do baseline da F6 (`rising_above_p95`/`below_baseline`/`within_baseline`); **sem histórico → `insufficient_history`** (§59). `firstBottleneck` = pior zona (§25). Limiares por-recurso **provisórios** (refinados por baseline/VPS spec). Determinístico (runtime injetável), sem LLM (§56). Rota master `GET /api/admin/capacity-headroom`. `test:capacity-headroom` (11 checks). **0 tabelas novas, 0 flags novas**, 0 breaking changes. Output útil em produção só depois do baseline acumular (§103).
- **F6 — FECHADA (motor de baseline/anomalia).** `src/server/PlatformBaselineService.ts` + tabela **global** `platform_health_snapshots` (o "Platform Health Event" que a auditoria apontou como faltante, §47 — sem `organization_id`, molde do `research_usage_log`). NÃO precisa de baseline pronto: é o motor que **começa a acumular** o baseline a partir do que o F2-F5 já observam. `capture` grava só **agregado** (uma linha por métrica, §11 — nunca raw) com seasonality bucket (dow/hora SP, §33); Scheduler faz captura horária + retenção de 90 dias (§19), só com a telemetria ligada. `baseline(metric)` deriva média/p50/p95/desvio (com filtro sazonal opcional) e — **§59/§103** — declara `insufficient_history` quando há menos de 8 amostras (não inventa). `anomalies()` sinaliza desvio sustentado (z≥2/≥3) como **HIPÓTESE com confiança** (`basis:correlation`, §35), nunca veredito; métrica sem variância não gera falso positivo. Determinístico (`at`/`now` injetáveis), sem LLM (§56/§57). Rotas master `GET /api/admin/platform-baseline` e `GET /api/admin/platform-anomalies`. `test:platform-baseline` (10 checks). Regressão `operational-health`/`dependency-health`/`platform-telemetry-contract` PASS. **1 tabela nova (agregada, global), 0 flags novas** (reusa `platform_telemetry_enabled`), 0 breaking changes.
