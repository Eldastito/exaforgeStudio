...
**Estado:** Em implementação — **F0–F9 FECHADAS (+ correlação de causa provável). O baseline/forecast SÓ produz resultado útil após dias acumulando em produção (§103); a fatia HOST/INFRA da F2 e o SLO por jornada (F3.4) dependem de dados de AMBIENTE**
