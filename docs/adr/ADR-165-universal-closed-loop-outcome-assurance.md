...
| **F13** ✅ | **`scripts/test-outcome-assurance-hardening.ts` (`test:outcome-assurance-hardening`, 8 checks) codifica como REGRESSÃO transversal os guardrails RN-OA: DONE≠assured (RN-OA-1), null≠zero (RN-OA-2), read-only não muda FSM (RN-OA-3), event_key idempotente (RN-OA-5), resolver determinístico via system-of-record (RN-OA-6), correção nasce awaiting_approval (RN-OA-9). Fecha o ADR-165.** | padrões existentes |
