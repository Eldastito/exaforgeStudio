...
| **F7 ✅** | `ResearchResult` ganha `evidenceMode` (`model_knowledge`/`live`) + `sourceEvidence[]` (`{url,title,tier A/B/C,retrievedAt,freshness}`); Stub e LLM se rotulam `model_knowledge` (fontes citadas → tier C, sem `retrievedAt`); `runResearch` persiste a procedência no conteúdo (observável) | ESTENDER | `test:research-provenance` (12) |
| **F8 ✅** | `LiveSearchResearchProvider` (busca VIVA atrás do MESMO contrato/registry, RN-EI-7) — `evidenceMode:'live'`, fontes tier B com `retrievedAt`; opt-in por `EXTERNAL_RESEARCH_SEARCH_URL`; SEM vendor → cai no stub `model_knowledge` (honesto, RN-EI-6); budget/master-only impostos pelo `runResearch` | CRIAR | `test:live-search-provider` (14) |
| **F9 ✅** | `assessQuality` gates 2.0: GROUNDING bloqueia `live` sem fonte (`ungrounded_live`, RN-EI-5); `model_knowledge` não exige fonte (0-regressão); avisos não-bloqueantes `model_knowledge_only`/`low_source_diversity`/`stale_sources`/`high_churn_vs_prior` (contradição vs base); `curate` passa a base fresca pro gate de contradição | ESTENDER | `test:research-curation-gates` (11) |
| **F10 ✅** | `publish` preenche `tenantTerms` (antes `[]`, achado (f)) com nome/CNPJ/telefone/e-mail do tenant de origem via `tenantTermsFor`; `assertNoTenantData` LANÇA se o nome do tenant vazar pro compartilhado (§94/§129); sem contexto de org → `[]` (0-regressão) | ESTENDER | `test:anonymize-tenant-terms` (9) |
| **F11 ✅** | `ResearchNeedService.detect` — temas ativos (sinais abertos) × `vertical_intelligence` fresca → lacunas mapeadas na taxonomia `(vertical,topic,region,timeframe)`; prioriza por severidade; sem vertical → honesto; NÃO roda pesquisa (RN-EI-4); rota `GET /research-needs` | COMPOR | `test:research-need` (12) |
| **F12 ✅** | `ContextualFusionService.fuse` — por tópico alinha interno + histórico assegurado + externo (com `evidenceMode`); força CATEGÓRICA strong/moderate/weak (assured+live→strong); NUNCA soma bases (CA35); procedência listada + caveats; rota `GET /context-fusion` | ESTENDER | `test:contextual-fusion` (14) |
| **F13 ✅** | `LearningMetricsService.metrics` — KPIs DERIVADOS por query: inventário por status, procedência (manual×assured), `assuredCoveragePct`, estados, `avgAssuredEffectiveness`, drift (dormância); percentuais null sem denominador (RN-EL-5); rota `GET /learning/metrics` | ESTENDER | `test:learning-metrics` (11) |
| **F14 ✅** | `test:enterprise-learning-hardening` codifica os RN-EL/RN-EI como regressão transversal + runbook `docs/runbook/learning-operacao.md`. **FECHA o PRD 9.** | CRIAR (teste) | `test:enterprise-learning-hardening` (12) |
