...
| F1 | `event_key`+idempotência em `recordOutcome`; ligação `OutcomeAssurance(assured)→recordOutcome` | ESTENDER/COMPOR | `test:enterprise-learning-assured` |
| F2 | `assuredEffectiveness` (recorte só-`assured`) separado | ESTENDER | `test:assured-effectiveness` |
| F3 | Learning Episode (read model derivado) | COMPOR | `test:learning-episode` |
| F4 | Preencher `historicalEvidence[]` no `EvidencePackageService` | ESTENDER | `test:historical-evidence` |
| F5 | `learningPrior` no `DecisionEngine` (explicável/reversível) | ESTENDER | `test:learning-prior` |
| F6 | Intervalo de confiança (Wilson) p/ taxa binária | CRIAR (mín.) | `test:effectiveness-interval` |
