...
| F1 | **Channel Capability Contract** — `SocialChannelProvider` (contrato+registry+capabilities) espelhando `ReputationProvider` | CRIAR (mín.)/COMPOR | `test:social-channel-contract` |
| F2 | **Social Connection Hub** — estende Canais e IA: OAuth, health, capabilities, permissões, card observável | ESTENDER | `test:social-connection-hub` |
| F3 | **Primeiro provider real** — escolhido na F0 por disponibilidade de API; vertical slice ponta-a-ponta | CRIAR (mín.) | `test:social-provider-<x>` |
| F4 | **Social Analytics Ingestion** — posts + analytics próprios (job no `JobQueue`/`Scheduler`) | ESTENDER/COMPOR | `test:social-analytics-sync` |
