...
| F11 | ✅ FECHADA — Inventory/Product Opportunity: `ProductOpportunityService` cruza `products_services` × `inventory_items.avg_cost` × desempenho pra achar o produto que MERECE conteúdo — em ESTOQUE, ALTA MARGEM (`price−avg_cost`), VENDENDO POUCO — e publica `product_opportunity` na espinha (`business_signals`, D7/§37 — sem tabela paralela). Dinheiro role-gated (RN-CG-06): `candidates()` (rota owner/admin) traz margem/custo ABSOLUTOS; o SINAL só carrega `marginBand` QUALITATIVO (nunca R$). `impactAmount=null` (RN-CG-03); `basis='hypothesis'`; grounding (sem custo/estoque → não entra, RN-CG-09); idempotente por dedupe. `pass()` no Scheduler. Rotas `GET /api/social/product-opportunities` + `POST /product-opportunities/match`. `test:product-opportunity` 15. | — |
| F12 | ✅ FECHADA — Growth Goal metric + goal↔content: `BusinessGoalService.METRICS` (registro schema-free) ganha `content_revenue` (BRL — só `fact` do `content_sale_attributions` no mês, RN-CG-03) e `content_leads` (count do `content_lead_attributions` no mês), DERIVADOS por query (RN-004). A meta de negócio passa a poder ser "o que o CONTEÚDO gerou"; o Campaign Objective Contract (F2) liga a elas e rastreia a distância-à-meta (goal↔content fechado). +1 entrada no registro fecha o gap (sem tabela/modelo novo, §37). Dinheiro role-gated (mesma gating de `revenue`). `test:growth-goal` 12. | — |
| F13 | ✅ FECHADA — Fala Tu Growth Brief: `SocialProactivityService.growthBrief` COMPÕE (read-only, §37 — sem nova superfície) o "o que postar + impacto esperado + campeão" pro dono: o que postar (oportunidades de conteúdo F7 + de produto F11), o progresso das metas de CONTEÚDO (F12, distância-à-meta) e o campeão atual dos experimentos (F6/F9). HONESTO (org vazia → tudo vazio, headline null); produto sem R$ (marginBand qualitativo); dinheiro de meta é role-gated (rota owner/admin, RN-CG-06). `digest()` intacto (0-regressão — `test:social-proactivity` segue 8). Rota `GET /api/social/growth-brief`. `test:growth-brief` 13. | — |
| F14 | ✅ FECHADA — 2º provider social REAL: `FacebookChannelProvider` (envolve `FacebookService` — Graph API de Página) espelhando `InstagramChannelProvider` (§37 — mesmo contrato/registry, sem 2ª tela de credenciais RN-SI-05). Capability DESCOBERTA (RN-SI-06 — desconectado→vazias+not_connected, determinístico em CI); degrada honesto (`manual_required`/`capability_unavailable`); publish idempotente in-memory (RN-SI-08); analytics honestos (null≠0, RN-SI-12); ads DEFERIDO (§4). `providerFor` roteia `facebook`+provider=`facebook`→provider real (default stub, opt-in, 0-regressão). TikTok/YouTube/X/Ads seguem DEFERIDOS. `test:social-provider-facebook` 20 (regressão `test:social-connection-hub` 24 + `test:social-provider-instagram` 21 verdes). | — |
| F15 | ✅ FECHADA — Growth Autopilot (shadow): `GrowthAutopilotService` — postura SHADOW-first (`off`|`shadow`; `auto` REJEITADO — RN-CG-10, crescimento autônomo nunca vai direto pra execução). `plan()` PROPÕE (promover campeão F9, promover produto F11, criar conteúdo F7) mas NUNCA executa (RN-CG-08 — read-only, ZERO `decision_actions`; toda proposta nasce `requiresApproval:true`/`wouldExecute:false`). A execução governada é a F16. Produto sem R$ (RN-CG-06). Coluna `organization_settings.growth_autopilot_mode` (default `off`, opt-in). Rotas `GET /api/social/growth-autopilot` + `POST /growth-autopilot/mode`. `test:growth-autopilot` 13. | — |
| F16–F18 | ⬜ pendentes | — |
