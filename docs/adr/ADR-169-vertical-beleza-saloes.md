...
| Fatia | Escopo | Status |
| --- | --- | --- |
| **F0** | doc-only: `docs/prd/ANALISE-PRD-BELEZA-SALOES-vs-CODEBASE.md` + este ADR-169; Reuse Matrix, Impact/Dependency/Data Model/Migration/Regression/Flag/Rollback/Security/AI-Cost/E2E, RN-BS-01..12, plano F1–F18 | **em andamento (esta PR)** |
| **F1** | Registro da vertical: +1 chave em `VerticalKey`/`VERTICALS`/`CONSENT_BY_VERTICAL`/`FALLBACK_HIDDEN_BY_VERTICAL`; `test:beauty-registry` (semeia consent + overview) | pendente |
