...
**Estado:** **F0 MERGEADA (PR #1121)** · **F1 em andamento** — registro da vertical `beleza` como DADO: +1 chave em `VerticalKey`/`VERTICALS` (preset agenda + vendas/pagamentos + campanhas/cadencias + assinaturas + estudio + diretor/rie/execucao; ícone 💇; label "Beleza & Salões"), +1 entrada em `CONSENT_BY_VERTICAL` (dados_pessoais + comunicacoes + marketing — sem `dados_sensiveis` porque a foto do Simulador é escopo separado `hair_simulation` de F5+), +1 linha em `FALLBACK_HIDDEN_BY_VERTICAL` (esconde `clinica`/`escola`/`retail_floor` — a vertical reusa a agenda profissional/sala/especialidade da Clínica pelos SERVICES, sem expor a UI de prontuário). `test:beauty-registry` (42 checks) codifica: preset coerente, consent semeado sem sobrescrever, EntitlementService reconhece a chave em plans autonomo/growth/scale, overview lista módulos ativos, isolamento cross-tenant beleza↔varejo, todas as 9 verticais aplicam sem erro, e um check textual garante zero hardcoded do Studio Márcia (§17/§65). Regressão em `test:vertical-plan-intersection` (19), `test:lgpd-vertical-consent` (5), `test:entitlement-service` (49), `test:entitlement-hidden-via-blueprint` (28) — tudo verde. Achado F0: **~85% é COMPOR / REUTILIZAR / CONFIGURAR** — a operação essencial de salão (agenda com profissional/sala/especialidade, ciclos renováveis, ficha extensível, lembrete 24h, lista de espera, remarcação por WhatsApp) já existe canonicalizada como Clínica; o Simulador de Cabelo é o **Fashion Studio com prompt invertido** (preservar rosto/corpo, alterar cabelo — mesmo contrato `TryOnProvider`, mesma família de tabelas, mesma comparação lado a lado já pronta no frontend). O trabalho de F1+ é: (a) 1 chave em `verticals.ts` + 1 blueprint `beleza_salao_v1`; (b) 1 família `beauty_*` de tabelas espelhando `fashion_*` + 1 provider; (c) 3 conectores finos ao `products_services` (duração lida pela agenda, N:N profissional↔serviço, `maintenance_days` opcional); (d) 5 command handlers novos no MESMO registry; (e) endurecimento **transversal** de consent+quiet-hours+frequency-cap (aditivo, beneficia todas). Nenhum motor novo. Plano F0–F18. Análise em `docs/prd/ANALISE-PRD-BELEZA-SALOES-vs-CODEBASE.md`.
...
| **F1** | Registro da vertical: +1 chave em `VerticalKey`/`VERTICALS`/`CONSENT_BY_VERTICAL` + `EntitlementService.FALLBACK_HIDDEN_BY_VERTICAL`; `test:beauty-registry` 42/42 (preset coerente, consent semeado, EntitlementService reconhece chave, overview lista módulos ativos, cross-tenant isolado, todas as 9 verticais aplicam sem erro, zero hardcoded do Studio Márcia — §17/§65). Regressão: `test:vertical-plan-intersection` 19/19, `test:lgpd-vertical-consent` 5/5, `test:entitlement-service` 49/49, `test:entitlement-hidden-via-blueprint` 28/28, `tsc --noEmit` limpo | **em andamento (esta PR)** |
| **F2** | Blueprint `beleza_salao_v1` (via `BlueprintSeeder.INITIAL_BLUEPRINTS`); Studio Márcia como tenant via `POST /api/admin/organizations/:id/blueprint` (dados, nunca constante); `test:beauty-blueprint-piloto` | pendente |
| **F3** | Perfis `beauty_recepcao`/`beauty_cabeleireira`/`beauty_gerente` (seed condicional por blueprint); Sidebar reconhece a vertical (usar navigation-manifest já existente); `test:beauty-permissions` | pendente |
