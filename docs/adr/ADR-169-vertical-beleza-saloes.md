...
| **F4** | **Parte 1:** `AppointmentService.create` respeita `products_services.duration_minutes` quando `productServiceId` passado (fecha gap #1 da auditoria); precedência scheduledEnd explícito > duração serviço > slot org (fallback). Helper público `serviceDurationMin(orgId, productServiceId)`. Multi-tenant duro; NULL/0/negativo caem no fallback. **Parte 2:** tabela nova `professional_services` (D6 — evita mapear como especialidade, libera comissão nativa) + `ProfessionalServiceService` (link idempotente, unlink soft-off, isCapable, setForProfessional atômico com validação cedo). Só type='service' aceito, commissionPercent validado [0..100]. `test:beauty-service-duration` 25/25 + `test:beauty-professional-services` 52/52. Regressão: `test:clinic-agenda` 27/27, `test:comigo-agenda` 42/42, `test:clinic-episode-appointment` 34/34, `test:falatu-bridge-recon` 15/15, tsc limpo. `nextFreeSlots(serviceDurationMin)` DEFERIDO (fatia futura, escopo focado) | **em andamento (esta PR)** |
