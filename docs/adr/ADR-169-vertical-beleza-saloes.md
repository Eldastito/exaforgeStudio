...
| **F2** | Blueprint `beleza_salao_v1` publicado em `BlueprintSeeder.INITIAL_BLUEPRINTS` (baseVertical=beleza, min=start, default=growth, sem bundle, hidden=clinica/escola/retail/retail_floor/vms/prospect, quickStartPack=null); `inferBlueprintKeyFor("beleza", *) → beleza_salao_v1`; Studio Márcia atribuído por dado via `VerticalBlueprintService.assignToOrganization`; `test:beauty-blueprint-piloto` 54/54 (blueprint publicado + idempotente + shape, inferência + regressão, EntitlementService usa hidden DO BLUEPRINT com `source.verticalBlueprint`, migração de tenants beleza legados, cross-tenant piloto↔clínica isolado, zero hardcoded Studio Márcia — §17/§65). Regressão: `test:blueprint-seeder` 78/78 (dinâmico), `test:vertical-blueprint-service` 48/48, `test:entitlement-hidden-via-blueprint` 28/28, `test:beauty-registry` 42/42, tsc limpo | **em andamento (esta PR)** |
