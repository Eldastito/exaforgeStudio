...
| **F5** | 4 tabelas novas (`beauty_consents` + `beauty_visual_consultations` + `beauty_avatar_assets` + `beauty_reference_looks`) + 2 flags em `organization_settings`; `BeautyVisualConsultationService` (376L) espelha `FashionAvatarService` com `contact_id` (CRM canônico) — consent tipado 4 escopos SEPARADOS, consulta draft→ready→selected→scheduled, upload com quarentena+EXIF strip real via sharp, URL assinada `fileSigning` canônico escopo `beauty_private_media_v1` TTL 15min, isolamento de escopo, retenção configurável (default 30d, clamp 1..365), purga preguiçosa + `purgeExpired`, revoke `hair_simulation` apaga assets (LGPD Art.18), `safety_report_json` só flags. Aprovação manual em F5 (validação por IA vem em F6). `test:beauty-visual-consultation` 71/71. Regressão: `test:fashion-avatar` 44/44, `test:beauty-registry` 42/42, `test:beauty-blueprint-piloto` 54/54, `test:beauty-profiles` 75/75, `test:beauty-service-duration` 25/25, `test:beauty-professional-services` 52/52, tsc limpo. Rota `/api/public/beauty/media` referenciada mas ainda não montada — F6 monta junto com a UI de simulação | **em andamento (esta PR)** |
