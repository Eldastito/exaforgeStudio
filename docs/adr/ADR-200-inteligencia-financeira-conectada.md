...
**Estado:** FECHADO — F1–F5 em produção (Balanço · Fluxo de Caixa indireto · Conexão + sinal `lucro_sem_caixa` · UI "Leitura do CFO" · hardening + narrativa LLM + runbook). Read-model derivado, aditivo, zero tabela nova. Runbook em `docs/runbook/financas-conectadas-operacao.md`.
**Data:** 2026-09-08.
...
- **F4 — UI** no menu Relatórios (ou um card "Leitura do CFO"): três demonstrativos + a narrativa da conexão; disclaimer sempre; dinheiro role-gated (§73). Sem tela concorrente ao Motor de Caixa. ✅ **em produção** — card **"Leitura do CFO"** no topo da seção financeira do Relatórios (`ReportsPanel`): três pílulas (Lucro competência × Caixa que entrou × Preso no giro) + banner "lucrou mas o caixa não veio" quando material + a narrativa da ponte + a decomposição lucro→caixa + caveats + disclaimer. Lê `GET /api/dre/connected` (owner/admin, §73). UI-only sobre o endpoint testado da F3; tsc+build verdes.
- **F5 — Hardening + narrativa LLM opcional** (D6) + runbook: `test:connected-financials-hardening` codifica RN-FIN-1..8; `docs/runbook/financas-conectadas-operacao.md`. ✅ **em produção — FECHA o ADR-200** — `ConnectedFinancialsService.narrateAsync` (D6): reescreve a leitura do CFO pela IA GROUNDED nos números derivados (RN-FIN-8, nunca inventa; fallback determinístico sem IA — 0-regressão), rota `GET /api/dre/connected/narrative`; `test:connected-financials-hardening` (13) codifica RN-FIN-1..8 sobre os serviços reais + a fiação de produção (serviços importáveis, 4 rotas montadas, `pass()` no Scheduler, testes registrados, runbook presente); runbook `docs/runbook/financas-conectadas-operacao.md`.
