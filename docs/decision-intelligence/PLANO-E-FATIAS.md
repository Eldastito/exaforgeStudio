...
- **DI-4.3 `[x]` ENTREGUE** — `EvidencePackageService` preenche o slot `externalEvidence[]` com as contextualizações **frescas** da org (read-only, isolado, vazio sem opt-in/sem dado). `DecisionEngine` consome inteligência externa do nicho **só em L3+** (perfil `externalResearch` do DI-1) via `ResearchBrokerService` read-only — **nunca dispara pesquisa** (o broker não chama provider); L0/L1 não consomem. Advocate cita a inteligência de mercado quando disponível. Teste: `npm run test:decision-intelligence-di4-evidence` (12 checks — hit L3, read-only, freshness, opt-out, isolamento).
- **DI-4.5 `[x]` ENTREGUE (lembrete semanal)** — coerente com o provider manual: o Scheduler (ADR-074) roda 1×/semana `VerticalIntelligenceReminderService.maybeWeeklySweep()`, detecta nichos **com consumidores** cuja pesquisa está vencendo/vencida e publica um **lembrete** no `business_signals` (domain `platform`) para o admin re-colar — **nunca roda pesquisa sozinho**. Auto-resolve quando re-colado (chave por episódio de vencimento). Toggle liga/desliga (`platform_settings`, ligado por padrão). Rotas master `GET/PUT /research-refresh-due` + banner no painel DI-UI-1. Teste: `npm run test:decision-intelligence-di4-reminder` (13 checks).
- **DI-4.4 `[x]` ENTREGUE (provider MANUAL)** — decisão do dono: o admin master **cola** a pesquisa do nicho (`VerticalIntelligenceService.runManual` + rota `POST /vertical-intelligence/manual`, master). Sem rede externa — o caminho mais seguro. Mesmo filtro de anonimização; custo zero (não toca no orçamento). Interface `ExternalResearchProvider` fica pronta para um web-search real no futuro. Teste: `npm run test:decision-intelligence-di4-manual` (10 checks — cola, anonimização do texto, bypass do orçamento, consumo do tenant, dedup, validação, sem org).
...
- **DI-UI-3 `[x]` ENTREGUE** — aba **"Analisar decisão"** no Diretor IA (`ExecutiveView`, ADR-152 D8: aba, não tela nova). O empresário descreve a decisão (título/tipo/valor/retorno/premissas) e recebe: nível **L0–L4** (DI-1), **cenários**, **Pre-Mortem** (riscos + mitigação + indicador), **Red Team** (premissas a validar), **Advocate** e **recomendação advisória** (o gate real segue no RBAC). Consome `POST /api/decision-intelligence/analyze`.
