...
| Promessa agenda nova verificação | [~] F4b.2 sinal + humano | F4b.4 (nova) | F4b.2: sinal `collection:reply_promise:${receivableId}` é publicado (attention) → dono vê na lista + decide re-cobrar. F4b.4 (aditiva) — agendamento automático via `SchedulerActionCommandHandler` fica pra próxima fatia |
