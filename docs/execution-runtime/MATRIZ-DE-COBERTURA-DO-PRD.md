...
| Cadência executada pelo Runtime | [x] F4b + F4b.2 + F4b.3 | F4b.3 | T1 (send_reminder) no F4b + intent router no F4b.2 + T2 (firme, D+3 default) e T3 (aviso de negativação, D+7 default) autônomas via `CollectionCadenceService.tickAll` no `Scheduler.collectionCadencePass`. Opt-in por org (`collection_cadence_enabled=1`) + thresholds configuráveis. Resposta do cliente pausa cadência. Verificado por `test-cobranca-cadencia-multitentativa.ts` (38/38) |
| Respostas alteram fluxo | [x] F4b.2 | F4b.2 | `CollectionIntentClassifier.classify` mapeia 10 intents PT-BR do §13.4 via `chat` do `llm.ts` (JSON mode + enum whitelist). `CollectionReplyService.tryHandle` publica `business_signal` categorizado + reply canned por intent (severity varia: `dispute/churn` = risk; `promise/claims_paid/…` = attention; `callback_later/unknown` = info). Verificado por `test-cobranca-intent-classifier.ts` (35/35) |
| Promessa agenda nova verificação | [x] F4b.4 | F4b.4 | Classifier extrai `promiseDate` do LLM (ex: "amanhã", "sexta"). `CollectionPromiseService.create` grava em `collection_payment_promises`. `Scheduler.collectionPromiseCheckPass` executa re-check no dia: se receivable pago → mark fulfilled + resolve sinal `reply_promise`; se aberto → WhatsApp follow-up ("combinamos pra {data}, deu pra acertar?") + mark broken + sinal `promise_broken` severity=risk. Verificado por `test-cobranca-promise-recheck.ts` (33/33). `SchedulerActionCommandHandler` genérico fica aspiracional — reuso em F4c revisará |
...
| Nova tentativa programada | [x] F4c.3 (approved_execution) | F4c.3 | `SalesRecoveryFollowupService.tickAll` (Scheduler) varre touches sem reply há N dias e PROPÕE 2ª/3ª msg com tom mais suave por tentativa (via `attemptNumber` propagado ao gerador e ao dedupe key). Cada tentativa passa por dono aprovar (G-4c.3-1). Opt-in duplo (`sales_recovery_followup_enabled=1`). Máx 3 tentativas. **Modo autonomous continua BLOQUEADO em decisão #4 LGPD.** Verificado por `test-sales-recovery-followup.ts` (32/32) |
...
| Receita com evidência | [x] F4c.4 | F4c.4 | `SalesRecoveryAttributionService.tickAll` no `Scheduler` escaneia `ticket_stage_logs` com `to_stage='ganho'` cujo ticket teve touch de recuperação em janela (default 30d). Calcula ticket_value com precedência clara (`orders` fact → `quotes` estimate → `contacts.avg_ticket` estimate → skip se zero). Grava outcome F3.1 `revenueRecovered=<value>` com `basis` correto (ADR-085 D4). Opt-in (`sales_recovery_attribution_enabled=1`). Dedupe forte via UNIQUE(org, ticket, stage_change_at) — reversão ganho→open→ganho atribui 2×. Verificado por `test-sales-recovery-attribution.ts` (25/25) |
