...
- **Fase 5 (P2) FECHADA** — 5A probe leve e autenticado `GET /api/health/ping` (`HealthProbeService`, CONN-003) ✅ · 5B+5C modelo de 4 estados (`online`/`realtime_degraded`/`api_degraded`/`offline`) derivado de rede×API×socket + texto honesto ("Tempo real reconectando — consultas e salvamentos continuam") + polling do probe + diagnóstico expandido no chip (internet · API estado/latência/último sucesso · tempo real · Alterdata última sync · comandos pendentes) — CONN-001/002/004 ✅ · 5D salvamento em degradação (CONN-005): auditoria mostrou que o outbox silencioso só carrega mensagem/captura (WhatsApp/FalaTu/Comigo), nunca operação financeira/comissão (que são `apiFetch` diretos com resposta do servidor) — invariante codificado como regressão + lógica pura extraída p/ `src/lib/connectivity.ts` e testada (`test:connectivity`) ✅. **Fase 5 FECHADA.**
- **Fase 6 (operacional) — material entregue** — runbook do piloto controlado em `docs/runbook/toulon-piloto-homologacao.md`: mapa das 5 fases × reversão, realidade dos flags (default seguro + rollback por PR; kill-switch de runtime como follow-up opcional), pré-requisitos por loja, estágios do rollout (Avenida Brasil → 2ª loja → todas → 7 dias → aceite), roteiro de homologação por loja, gates de monitoramento (p95 ≤ 2 s etc.), Definition of Done (§18), perguntas de homologação (§17), riscos/mitigação (§16) e procedimento de rollback. A execução do piloto e o aceite formal são da TOULON. Kit de execução (formulários a preencher) em `docs/runbook/toulon-piloto-kit.md`: estado do rollout, respostas de homologação (§17), pré-requisitos e checklist POR loja (roteiro §5, PASS/FAIL/evidência), log de monitoramento de 7 dias, índice de evidências, sign-off da DoD (§18), bloco de aceite formal e referência rápida dos kill-switches.
- **Fase 6B (kill-switches de runtime)** — `RetailFeatureFlagService` + 2 colunas em `organization_settings` (DEFAULT 1 = comportamento novo, 0-regressão): `retail_business_date_v1` (0 → data comercial volta ao UTC, choke point em `BusinessTimeService.businessDate`) e `retail_analytics_resolved_products_v1` (0 → CMV/precificar/mais-vendidos voltam ao LIKE-prefix; ramo resolvido vira `1=0`, ramo LIKE vira `1=1`). Rotas owner/admin `GET/PUT /api/retailops/feature-flags[/:key]`; desligar invalida o cache. Equivalência numérica ligado×desligado + reversão da data comercial em `test:retail-feature-flags` (13). Reverte no piloto sem deploy, por org. ✅
