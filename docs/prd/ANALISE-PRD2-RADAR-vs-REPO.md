...
| --- | --- | --- |
| **Anomaly framework genérico** (baseline + deviation + min-sample + cooldown + TTL) como primitiva reutilizável | Hoje é código inline repetido (`ConsumptionSignalPublisher` ×1.5, `RetailFloor.conversionDrop` 20%/min20). §22/§25/§26 | F4 |
| **Detector registry** (contrato §67 + defaults por vertical §89-90) | Detectores hardcoded importados no Scheduler; sem registry | F4 |
| **Correlation Engine** (N sinais → 1 situação, multi-evidência) | Inexistente. Só há dedupe (mesmo evento). §16-20 pedem correlação (evento≠evento). **F3.1 ENTREGUE:** `SignalCorrelationService.clusters` — confiança ALTA (mesmo `(subject_type,subject_id)`, multi-domínio, janela), derivado, evidência preservada. `test:signal-correlation` (10 checks). Falta F3.2 (surface no attention) + F3.3 (média/baixa) | F3 |
...
| CA4 correlação sem destruir evidência | 🟡 **F3.1 (alta confiança) ✅**; falta média/baixa | F3.3 |
| CA5 framework de anomalia | ❌ inline-repetido | F4 |
| CA6 baseline | 🟡 2 casos inline | F4 |
