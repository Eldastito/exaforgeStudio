...
- **0-regressão comprovada:** `test:scheduling-overlap` trava a tabela-verdade **e** a equivalência exata com a expressão legada em 2401 combinações; regressões verdes (clinic-agenda 27/27 · comigo-agenda · clinic-reschedule 42/42 · professional-availability 27/27 · reservation-service · professional-booking 23/23). **F1.5 segue aberta** (Onda 3).

### 10.6 F1.3 (SignalReactionPolicy) — RECLASSIFICADA: motor de policy → NOT_NEEDED/DEFER (DUP-005 superdimensionado)

Auditoria dos dois "consumidores concorrentes" de sinal mostrou que **não há política de reação faltante nem duplicação verbatim** — são duas reações ortogonais, cada uma já governada e conservadora:

| Consumidor | Reação | Mapa | Governança | Idempotência |
| --- | --- | --- | --- | --- |
| `SignalProcessRouterService` | sinal → **inicia processo** de remediação | `TRIGGER_MAP` explícito + allowlist de processos maduros | opt-in DUPLO (`execution_runtime_enabled` + `signal_auto_trigger_enabled`) | `acknowledge` (sai da varredura) |
| `MissionProactiveService` | sinal → **propõe missão** (rascunho) | `shapeForSignal` (regex domínio/tipo → forma) | shadow-first (`off`/`shadow`/`suggest`, nunca `auto`) | dedup `signal:<id>` em `baseline_state` |

- **DUP-005 estava superdimensionado:** há *semelhança estrutural* (ambos leem `business_signals` abertos e mapeiam `domínio:tipo`→reação), mas *semântica legítima diferente* (remediar via processo × propor meta). Unificar atrás de uma "SignalReactionPolicy" única forçaria um mínimo denominador que nenhum dos dois quer (o router precisa de allowlist de processo + acknowledge; a missão precisa de modos shadow-first + derivação de forma + dedup por baseline).
- **Nada verbatim a extrair** (ao contrário do predicado de overlap da F1.2): as duas funções de mapeamento são código totalmente diferente. Criar a policy seria *inventar* uma abstração sem duplicação que a justifique — e `CREATE` é o último degrau da escada RE→EXTEND→COMPOR→MIGRAR→CRIAR.
- **Risco vs. valor:** os dois caminhos já são opt-in, determinísticos e não-executam-sozinhos (o router só cria instância em `detected`; a missão nasce rascunho). Um motor de enforcement por cima só adicionaria risco a caminhos que funcionam — e a própria auditoria (§7) marcou F1.3 como baixa prioridade / irrelevante ao F3.
- **Decisão:** F1.3 → **NOT_NEEDED/DEFER** (doc-only, sem código). Observação menor, não-acionável agora: `MissionProactiveService.scan` lê `business_signals` por SQL cru (filtros próprios de severidade/limit) em vez do accessor canônico `BusinessSignalService.list` — consistência a considerar se um dia a leitura de sinais for consolidada, mas não vale mexer sob "não quebrar o que funciona". **F1.5 segue aberta** (Onda 3), parcialmente bloqueada no mesmo dado de produção da F1.1.
