...
> **UI:** o painel **"Radar — Saúde"** (`RadarHealthView`, visível ao dono/admin da org e ao master de plataforma — mesmo perfil que os endpoints exigem) materializa `/health` + `/detector-budget` — verdito geral, volume por severidade, e por detector o status/frescor/calibração + a barra de consumo do budget de IA do dia. Pura leitura.
