...
| P2 | Webhook genérico marca pago com `paid` default `true` (sem status) e SEM conferir valor (`server.ts`); sem replay/dedup no path de pagamento (WhatsApp já tem `claimWebhookEvent`); Stone confia no corpo do POST. Exige o segredo do webhook da org (agora protegido pelo P1). | 🟠 **PRÓXIMA FATIA (SEC-F20):** exigir status pago explícito + conferir valor + `claimWebhookEvent('payment')` + Stone re-consultar a API. |
