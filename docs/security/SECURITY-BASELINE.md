...
| A16 | 🟡 P2 | ~~Dockerfile roda como **root**~~ — **✅ NÃO-ROOT (SEC-F23):** a EXECUÇÃO cai pro usuário `node` (uid 1000) — build segue como root (precisa do toolchain), só o runtime é sem privilégio. **Validar no deploy:** volume de DATA_DIR precisa ser gravável pelo uid 1000. Pendente (follow-up de menor risco, após validar o não-root): multi-stage p/ tirar toolchain (python3/make/g++) + devDeps da imagem de runtime. | `Dockerfile` |
...
| — | Cartão (PCI): nenhum PAN/CVV é gravado/logado — 100% delegado ao gateway (PIX MP + link Stone). Mercado Pago e Asaas RE-CONSULTAM a API antes de marcar pago (autêntico). | ✅ SEGURO |
...
| # | Sev | Achado | Evidência (`arquivo:linha`) |
| --- | --- | --- | --- |
| FE1 | 🟠 MED | ~~JWT cru + objeto de usuário completo em `localStorage` (`zappflow_token`/`zappflow_user`) — qualquer XSS exfiltra a sessão inteira.~~ — **✅ FASE 1 (SEC-F24, backend):** o login agora EMITE a mesma sessão num cookie **httpOnly** (`zf_session`, `SameSite=Strict`, `Secure` em prod) e o backend ACEITA o token do cookie além do header `Authorization` (header tem precedência → **0-regressão**, ninguém deslogado). Autenticação por cookie ganha defesa **CSRF por mesma-origem** (`Origin`/`Referer`) em métodos que mudam estado; `POST /logout` limpa o cookie; socket handshake também lê o cookie. **FASE 2 (frontend, pendente — exige validação em navegador):** SPA parar de guardar o token no `localStorage` e confiar só no cookie, atrás de flag, migrando sessões existentes sem logout. | `src/server/sessionCookie.ts`; `src/server/middleware/auth.ts`; `src/server/routes/auth.ts`; `server.ts`; `src/contexts/AuthContext.tsx:74-75` (fase 2) |
