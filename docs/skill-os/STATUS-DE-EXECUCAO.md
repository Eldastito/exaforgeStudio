...
- **Fase:** 11 — Evals + Shadow (em revisão/PR).
- **Última fatia:** F11 entregue (harness de eval determinístico + gate de regressão + shadow). F10 mergeada (#967).
- **Baseline:** `main` @ `12b4cf3` (pós F10).
...
- Aprovada a F11 → **Fase 12 (Canary + Production Readiness)** — ÚLTIMA fatia do PRD 4: rollout progressivo (development→tests→shadow→pilot→assisted→approved_execution→broader, §68) + rollback + kill switch (reusa flags opt-in + `execution_mode`) + runbook em `docs/skill-os/` + SLO/drills. ESTENDER `ProductionReadinessService`/`RadarHealthService`; CRIAR só o que faltar. Sem motor de rollout paralelo.

## Testes / CI

- `test:skillos-contracts` (31) + `-registry` (21) + `-resolver` (19) + `-reliability` (19) + `-model-router` (19) + `-grounding` (21) + `-planner` (18) + `-execution-bridge` (13) + `-observability` (31) + `-tenant-usage` (20) + `-evals` (35), determinísticos. Runtime governado (command-executor/decision-actions/runtime-execute-e2e) + runtime-operations (indicators) + consumption-topup + plan-fit-detector verde. AI-usage + billing + context + tenant-isolation verdes — 0 regressão.
