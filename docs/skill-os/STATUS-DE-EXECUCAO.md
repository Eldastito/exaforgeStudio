...
- **Fase:** 10 — Tenant Usage Experience (em revisão/PR).
- **Última fatia:** F10 entregue (visão de consumo de IA do tenant, §30-safe, reusa alerta existente). F9 mergeada (#966).
- **Baseline:** `main` @ `a56cdad` (pós F9).
...
- Aprovada a F10 → **Fase 11 (Evals + Shadow)**: eval contínuo determinístico em 2–3 skills maduras (pilotos §61: Collection Intent Classifier, Sales Recovery Message, Signal Investigation) — golden/baseline por skill (ESTENDE a convenção `scripts/test-*` + `goldenStringify`) + shadow runner (candidata×atual, sem efeito, host no Scheduler). CRIAR só o scorer; sem plataforma de ML. (Correção de rota: o audit sequencia F10=Tenant Usage → F11=Evals+Shadow → F12=Canary; a nota anterior tinha desviado.)

## Testes / CI

- `test:skillos-contracts` (31) + `-registry` (21) + `-resolver` (19) + `-reliability` (19) + `-model-router` (19) + `-grounding` (21) + `-planner` (18) + `-execution-bridge` (13) + `-observability` (31) + `-tenant-usage` (20), determinísticos. Runtime governado (command-executor/decision-actions/runtime-execute-e2e) + runtime-operations (indicators) + consumption-topup + plan-fit-detector verde. AI-usage + billing + context + tenant-isolation verdes — 0 regressão.
