...
- [x] Fase 7 (Recomendação IA) — Fatias 7.1 (motor) + 7.2 (score/gap) + 7.3 (cooldown LGPD §14 — Decisão #7 resolvida) + 7.4 (UI card) + 7.5 (chat cita — framing G-153-3 embutido) + 7.6 (funil admin cross-tenant `AdminUpgradeRecommendationsPanel`) + 7.7 (cleanup automático de cooldowns vencidos no `Scheduler.tick`) FECHADAS. Decisão #8 (chat framing) resolvida pelo padrão adotado (contexto passivo, sem tool call). Admin agora pode processar upgrade manual dos "accepted awaiting checkout" até Fase 5 automatizar via Asaas. Ledger `upgrade_recommendations` mantém-se limpo automaticamente (dismissed com cooldown vencido → expired).
