...
### Sessão 2026-08-05 (Fatia 3.3 — UI Master Admin evolui blueprint v1→v2 com preview de diff)

- **Fase:** 3 (Vertical Blueprints). Fatia 3.3 fecha a simetria arquitetural: blueprint publica imutável (F3.1, G-153-5) → seed inicial das 5 verticais (F3.2) → **agora Master Admin pode evoluir** com nova versão + preview de diff + publicar (F3.3) → migração das orgs opt-in (endpoint `POST /organizations/:id/blueprint` já existe da F3.1).
- **Itens executados:** 4 (2 novos métodos no service, 2 rotas admin, 1 painel `BlueprintsPanel` no `AdminMasterView`, 1 test suite dedicada).
- **Arquivos alterados:**
  - `src/server/VerticalBlueprintService.ts` — 2 novos métodos: `createNextVersion(sourceId, edits, actor?)` (clona config da versão-base + aplica edits parciais + auto-incrementa version via `createBlueprint` sem passar version; rejeita se source é deprecated) e `previewBlueprintDiff(sourceId, targetId)` (compara 2 blueprints direto SEM depender de org — retorna added/removed por categoria de módulo + commercialUpgrades + runtimePlaybooks + array de `scalarChanges` com {field, from, to} pra name/minimumPlanId/defaultPlanId/defaultBundleKey/quickStartPack).
  - `src/server/routes/admin.ts` — 2 rotas: `POST /api/admin/blueprints/:id/next-version` (body `{edits}`) e `GET /api/admin/blueprints/:id/diff?targetId=X`. Já gateadas pelo `requireMasterAdmin` do mount.
  - `src/features/AdminMasterView.tsx` — imports novos (Layers, Plus, ArrowRight, Minus). Novo `<BlueprintsPanel />` inline entre `UpgradeRecommendationsPanel` e `AuditLogsPanel`. Componente `DiffSection` helper. UX: agrupa blueprints por `key`, mostra latest version + status pill, expand pra ver histórico, botão "+ Nova versão" abre editor inline (nome + minimumPlanId + 3 textareas CSV pra required/optional/hiddenModules), botão "Criar draft + ver diff" cria a nova version e imediatamente renderiza o painel de diff com +/-/mudanças escalares, botão "Publicar (fica imutável)" final. Publicar exige confirmação.
  - `package.json` — script `test:blueprint-next-version`. CI descobre via `ci-shard.mjs`.
- **Arquivos criados:**
  - `scripts/test-blueprint-next-version.ts` — 25 checks. Cobre: createNextVersion a partir de published/draft, sem `edits.config` (config verbatim), edits parciais (merge shallow, arrays substituem), rejeita source deprecated, blueprint original permanece IMUTÁVEL após bumps, previewBlueprintDiff cobre módulos + commercialUpgrades + escalares (name/minimumPlanId/defaultPlanId/quickStartPack), NÃO depende de org, rota POST retorna 201 com draft, rota GET diff funciona + 400 sem `targetId`.
- **Testes executados:**
  - `npm run test:blueprint-next-version` → 25/25 PASS.
  - Regressão: `test:vertical-blueprint-service` (48/48), `test:blueprint-seeder` (70/70), `test:admin-upgrade-recommendations` (21/21), `test:entitlement-service` (49/49). Todos verdes.
  - `npx tsc --noEmit` limpo (após ajuste `Object.entries(byKey) as [string, any[]][]` — TS não infere direito).
- **Decisões micro:**
  - (i) **`createNextVersion` reusa `createBlueprint`** — não duplica validação (assertValidKey/assertValidModules/etc). Deixa auto-versionamento (MAX(version)+1) sob single responsibility em `createBlueprint`; `createNextVersion` só compõe o input com clone do source.
  - (ii) **Merge shallow, arrays substituem completamente** — motor de UI edita CSV; se admin quer só adicionar módulo à optional, tem que reescrever a lista inteira. Alternativa (merge profundo com concat) seria surpreendente porque não teria como remover módulo depois. Merge shallow é o único idiomático.
  - (iii) **Rejeita bumpar a partir de `deprecated`** — deprecated é aposentado; permitir gerar novas versões dele confundiria (v3 apareceria "abaixo" de um v2 deprecated na timeline). Se admin quer resgatar preset antigo, cria com key nova.
  - (iv) **`previewBlueprintDiff` NÃO depende de org atribuída** — diferente de `previewEntitlements` (que compara com a org current). Motivo: fluxo de F3.3 é "criei v2, quero ver o que mudou vs v1 ANTES de publicar" — sem `previewEntitlements` teria que atribuir a alguma org fake pra ver o diff. Novo método é o correto pro momento pré-publicação; `previewEntitlements` continua o correto pro momento pré-migração de org.
  - (v) **scalarChanges como array** — não como object com fields nomeados. Motivo: motor de UI renderiza em loop; array + `{field, from, to}` é mais uniforme. Se surgir necessidade de query por field, `find` sobre array pequeno é O(N) trivial.
  - (vi) **UI: editor inline, não modal** — segue padrão dos outros painéis do `AdminMasterView` (nenhum usa modal). Aparecer/desaparecer com border colorida deixa o contexto visual claro. Se crescer muito, refatoramos.
  - (vii) **CSV pra módulos, não checkboxes** — módulos são 30+ strings livres. Checkbox array seria mais amigável, mas exige lista canônica dos módulos disponíveis (que já existe em `OPTIONAL_MODULES` mas depende de outra tabela); F3.3 é MVP do editor. F3.4 pode adicionar autocomplete/pillbox se admin pedir.
  - (viii) **Publica direto da UI de diff** — botão "Publicar (fica imutável)" no rodapé do painel de diff. Alternativa (publicar só via lista de versões) forçaria admin a rolar tela pra baixo. Fluxo linear "editar → diff → publicar" na mesma seção.
  - (ix) **Editor mostra sourceVersion no header** — "Nova versão de {key} (baseada em v{sourceVersion})". Admin sempre sabe de qual versão veio (sourcing).
  - (x) **Não valida no frontend antes de submeter** — deixa o backend rejeitar via `assertValidModules`/`assertValidPlanId` e mostrar erro via toast. Motivo: single source of truth de validação; UI evita replicar lista de módulos válidos que já fica no backend.
- **Cross-service:** ADITIVO PURO. `VerticalBlueprintService` intocado nos métodos existentes; ganhou 2 novos. `admin.ts` mount de rotas intocado; ganhou 2 novas rotas. `AdminMasterView` mantém painéis existentes; ganhou 1 novo painel entre Upgrade e Audit. `EntitlementService` intocado (F1.4 continua consumindo blueprint.hiddenModules — que agora pode ser evoluído via UI, mas ainda em nova version imutável).
- **Resultado:** ciclo end-to-end de blueprints está fechado UX-e-arquiteturalmente. Master Admin agora **evolui** um preset sem SSH/CLI: abre painel → cria nova versão baseada na atual → revisa diff → publica → migra orgs opt-in (endpoint existente). Simetria G-153-5 (imutabilidade) preservada. Fase 3 do ADR-153 100% fechada operacionalmente.
- **Pendências criadas:** nenhuma nova. Elegíveis: F4.4 (gráfico temporal), F3.4 (editor completo de limits/features do blueprint — não só módulos), Fase 5 (bloqueada Decisão #2 ToS).
- **Próximo passo:** com Fases 3 e 7 100% fechadas, o funil natural continua: (a) **F4.4 — gráfico temporal aceitas vs dispensadas** na aba Plano e Expansões (usa histórico F4.3; insight sazonal pro dono); (b) **F3.4 — editor completo de blueprint** (limits/features/pricing, não só módulos); (c) desbloquear **Fase 5** (Decisão #2 jurídica). **Recomendo (a) F4.4** — pequena e visível pro dono (ele já vê o card F7.4 e o histórico F4.3; falta a tendência temporal). F3.4 é refactor UX pra caso rara (admin quer mexer em limits). Fase 5 depende de decisão externa.

### Sessão 2026-08-05 (Fatia 4.4 — gráfico temporal aceitas vs dispensadas)

- **Fase:** 4 (Interface: novos painéis). Fatia 4.4 fecha o trio da aba "Plano e Expansões": card pending (F7.4) → histórico linha-a-linha (F4.3) → **agora tendência temporal** (F4.4). Dono vê sazonalidade das próprias decisões — "aceitei mais em agosto que em julho" = pista pra ajustar oferta / motor de score.
- **Itens executados:** 4 (1 método novo no service + 1 rota nova + 1 componente Recharts + 1 test suite).
- **Arquivos alterados:**
  - `src/server/UpgradeRecommendationService.ts` — novo método `historyByBucket(orgId, {days?})` (~50 linhas). Query com `GROUP BY date(COALESCE(accepted_at, dismissed_at, updated_at))` filtrando `status IN ('accepted','dismissed')` na janela. Days clampado 7..180 (default 30). Retorna série alinhada aos últimos N dias (dias sem decisão = 0/0, sem gap) + totalAccepted + totalDismissed. Isolamento multi-tenant preservado (organization_id em toda query).
  - `src/server/routes/recommendations.ts` — nova rota `GET /api/billing/recommendations/history-chart?days=N` inserida ENTRE `GET /` e `GET /:id` (Express matching ordem — path específico antes do param).
  - `src/features/SettingsView.tsx` — imports: `BarChart3` do lucide + toda a família Recharts (`BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend`). 2 componentes novos: `TrendTooltip` (padrão visual do rie/TrendChart) + `RecommendationTrendChart` (~90 linhas, self-contained: fetch + loading skeleton + empty state + bar chart 2 séries emerald/zinc + resumo textual "N aceitas / N dispensadas nos últimos 30 dias"). Wiring: nova seção em `PlanoExpansoesPanel` entre o bloco de histórico (F4.3) e o CTA final. Só renderiza se `recHistory.length > 0` (não gasta pixel em orgs zeradas).
  - `package.json` — script `test:recommendation-history-chart`. CI descobre via `ci-shard.mjs`.
- **Arquivos criados:**
  - `scripts/test-recommendation-history-chart.ts` — 39 checks (17 casos). Cobre: empty org (série toda 0/0), days clamping (piso 7, teto 180, default 30), datas ordenadas cobrindo [now-29..now], accepted/dismissed hoje no último bucket, backdate 3d/10d aparece no bucket certo, múltiplas mesma data agregam, pending NÃO conta, expired (após sweep F7.7) NÃO conta, backdate 60d fora de days=30 mas dentro de days=90, totalAccepted/Dismissed batem com soma da série, isolamento multi-tenant (orgB não vê decisões de orgA), rota HTTP 200 com org / 401 sem, payload shape correto.
- **Testes executados:**
  - `npm run test:recommendation-history-chart` → **39/39 PASS**.
  - Regressão: `test:upgrade-recommendations` (47/47), `test:admin-upgrade-recommendations` (21/21), `test:plan-fit-detector` (65/65), `test:scheduler-plan-fit-cooldown-expire` (10/10), `test:executive-plan-recommendations-block` (23/23). Todos verdes.
  - `npx tsc --noEmit` limpo (backend + frontend).
- **Decisões micro:**
  - (i) **Bucket = `date(COALESCE(accepted_at, dismissed_at, updated_at))`** — accepted_at e dismissed_at são timestamps de decisão (populados em `accept()`/`dismiss()`). COALESCE cobre linhas legadas hipoteticamente sem timestamp de decisão via fallback pra updated_at, sem quebrar o filtro (updated_at é sempre populado). O status IN filter garante que expired/pending nunca cheguem no bucketize.
  - (ii) **SÓ `accepted` e `dismissed` contam** — não `pending` (ainda não decidido — não faz sentido colocar no gráfico de decisões), não `expired` (housekeeping automático F7.7, não é decisão humana). Coerente com a semântica do gráfico: "decisões explícitas do dono ao longo do tempo".
  - (iii) **Days clampado 7..180** — 7 é o mínimo pra ter sinal visual (janela mais curta seria só ruído); 180 casa com o teto do cooldown ladder (F7.3 RN-153-F7.3-002). Default 30 bate com a 1ª pisada do cooldown — o dono vê o efeito de cada dispensa/aceite no mês.
  - (iv) **Série alinhada preenche dias faltantes com 0/0** — mesma abordagem do `buildDailySeries` do `AnalyticsService.ts`. Frontend não precisa preencher gap; renderiza direto. Sem esse preenchimento, um mês com decisões esparsas viraria 3 barras aleatórias sem eixo temporal claro.
  - (v) **Empty state no frontend** (`totalAccepted + totalDismissed === 0`) — mostra frase amigável em vez de BarChart vazio (visualmente feio + confuso). Skeleton animado enquanto carrega (padrão do resto do repo).
  - (vi) **Seção só aparece se `recHistory.length > 0`** — evita gastar pixel em orgs virgens (mesma condição do bloco F4.3 acima). Se o dono nunca decidiu nada, também não faz sentido mostrar o gráfico.
  - (vii) **Cores: emerald `#10b981` (accepted) + zinc `#a1a1aa` (dismissed)** — bate visualmente com os badges do `RecommendationHistoryRow` (F4.3) usando `emerald-300` e `zinc-300`. Consistência cross-componente na mesma aba.
  - (viii) **Rota inserida entre `/` e `/:id`** — Express match order importa: se ficasse depois de `/:id`, o path `/history-chart` viraria `req.params.id === "history-chart"` e cairia no getById → 404. Diretriz de todas as rotas Express do repo.
  - (ix) **Teste com mount enxuto de express + fetch global** — padrão idêntico ao `test-admin-upgrade-recommendations.ts`. Não usa createApp/JWT real (auth JWT é testado em outros suítes) — middleware fake injeta `req.organizationId` do header `x-test-org` pra isolar o comportamento da rota.
  - (x) **Teste 11 (expired NÃO conta) faz sweep real via `expireOldCooldowns`** — reproduz o loop de produção (F7.7) em vez de UPDATE direto pra `status='expired'`. Garante que a interação real service→service não regride.
- **Cross-service:** ADITIVO PURO. `UpgradeRecommendationService` intocado nos métodos existentes — só ganhou `historyByBucket`. `routes/recommendations.ts` intocado nas 4 rotas anteriores — só ganhou 1 nova. `PlanoExpansoesPanel` mantém histórico F4.3 + card F7.4 + CTAs anteriores. Nenhum consumidor existente muda.
- **Resultado:** aba "Plano e Expansões" fecha o trio completo — dono vê recomendação atual (F7.4) + histórico linha-a-linha (F4.3) + **tendência ao longo do tempo (F4.4)**. Insight sazonal claro sem depender do admin. Fase 4 quase 100% fechada (falta só F4.1 explícita — mas F1.3 já entregou o ModulesPanel de 3 seções que era o objetivo).
- **Pendências criadas:** nenhuma nova. Elegíveis: F3.4 (editor completo de blueprint — limits/features), F4.1 formal (revisão do ModulesPanel — cosmético; F1.3 já entregou o comportamento), Fase 5 (bloqueada Decisão #2 ToS).
- **Próximo passo:** com aba "Plano e Expansões" completa UX-e-arquiteturalmente, os naturais são: (a) **F3.4 — editor completo de blueprint** (permite admin mexer em limits/features/pricing sem SSH); (b) desbloquear **Fase 5** (assinatura Asaas real, bloqueada por Decisão #2 jurídica); (c) mini-melhorias UX (toggle 7d/30d/90d no gráfico F4.4, cores customizáveis, export CSV). **Recomendo (a) F3.4** — última loose-end de UX admin que ainda depende de SSH pra caso raro (mudança de limits). (c) é polish sem impacto de vendas. Fase 5 depende de decisão externa.
