...
const SEED: SeedItem[] = [
  ...
  },
  {
    ...
    title: "Enterprise Intelligence Kernel / CONTROLER (operacional)",
    domain: "governance",
    summary: "Camada operacional transversal do Diretor/CEO (ADR-135): BusinessSnapshotV2 + adapters de finanças/vendas/estoque/compras/operação/tarefas + ExecutiveAdvisorService. Alimenta o CEO Operating Layer (ADR-190) e /api/decision-intelligence. Split do §5.3: metade 'learning' movida pra ENTERPRISE_LEARNING (ADR-166).",
    source_of_truth: "ADR-135",
    target_status: "PRODUCTION",
    sources: [
      ...
      { source_type: "file", title: "BusinessSnapshotV2Service + adapters", source_reference: "src/server/BusinessSnapshotV2Service.ts" },
      { source_type: "file", title: "ExecutiveAdvisorService", source_reference: "src/server/ExecutiveAdvisorService.ts" },
      { source_type: "file", title: "ExecutiveBusinessSnapshotService", source_reference: "src/server/ExecutiveBusinessSnapshotService.ts" },
    ],
  },
  {
    evolution_key: "ENTERPRISE_LEARNING",
    title: "Enterprise Learning (outcome→prior via PatternMemory)",
    domain: "governance",
    summary: "Split do §5.3 (era metade de ENTERPRISE_INTELLIGENCE_CONTROLER): loop de aprendizado outcome→learning→prior→melhor decisão. ADR-166 F0–F14 fechado. PatternMemoryService (motor único, RN-EL) + LearningEpisodeService + LearningMetricsService + PatternLearningFromAssuranceService + external intelligence com procedência (ResearchNeedService, ContextualFusionService, LiveSearchResearchProvider).",
    ...
    sources: [
      ...
      { source_type: "file", title: "PatternMemoryService (motor único genérico)", source_reference: "src/server/PatternMemoryService.ts" },
      { source_type: "file", title: "LearningEpisodeService (read model per pattern)", source_reference: "src/server/LearningEpisodeService.ts" },
      { source_type: "file", title: "LearningMetricsService (KPIs derivados)", source_reference: "src/server/LearningMetricsService.ts" },
      { source_type: "file", title: "PatternLearningFromAssuranceService (outcome→learning bridge)", source_reference: "src/server/PatternLearningFromAssuranceService.ts" },
      { source_type: "file", title: "Runbook Learning Operação", source_reference: "docs/runbook/learning-operacao.md" },
    ],
  },
  {
  ...
  },
  {
    ...
    domain: "governance",
    summary: "Design de Enterprise Intelligence anterior à ADR-166. Absorvido pela iniciativa ENTERPRISE_LEARNING (que é exatamente o loop outcome→prior que a pré-ADR-166 esboçava sem fechar). Camada operacional/kernel do CONTROLER (ADR-135) segue viva como iniciativa separada (ENTERPRISE_INTELLIGENCE_CONTROLER).",
    target_status: "SUPERSEDED",
    superseded_by: "ENTERPRISE_LEARNING",
    sources: [
