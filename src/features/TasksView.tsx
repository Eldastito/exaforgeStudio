...
type Task = {
  ...
  created_at: string; updates?: Update[];
};
...
export function TasksView() {
  ...
  const userName = (id?: string | null) => users.find(u => u.id === id)?.name || users.find(u => u.id === id)?.email || '—';
  ...
  return (
    <div className="flex-1 overflow-auto p-6 bg-zinc-950">
      ...
      {detail && <DetailDrawer task={detail} users={users} userName={userName} onClose={() => setDetail(null)} onChanged={(t) => { setDetail(t); load(); }} onMove={move} />}
    </div>
  );
}
...
function DetailDrawer({ task, users, userName, onClose, onChanged, onMove }: {
  task: Task; users: OrgUser[]; userName: (id?: string | null) => string;
  onClose: () => void; onChanged: (t: Task) => void; onMove: (id: string, s: Task['status']) => void;
}) {
  ...
  const reassign = async (userId: string) => {
    try {
      const r = await apiFetch(`/api/tasks/${task.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ assignedTo: userId || null }) });
  ...
  const addNote = async () => {
    ...
    setBusy(true);
    try {
      const r = await apiFetch(`/api/tasks/${task.id}/notes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: note }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Falha');
      setNote(''); onChanged(d);
    } catch (e: any) { toast.error(e.message); } finally { setBusy(false); }
  ...
  const runAssist = async () => {
    ...
    try {
      const r = await apiFetch(`/api/tasks/${task.id}/assist`, { method: 'POST' });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Falha');
      setAssist(d.assist || '');
    } catch (e: any) { toast.error(e.message); } finally { setAssisting(false); }
  };
  const cancelTask = async () => { onMove(task.id, 'cancelada'); onClose(); };
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={onClose}>
      <div className="h-full w-full max-w-[440px] bg-zinc-900 border-l border-zinc-800 overflow-auto p-5" onClick={e => e.stopPropagation()}>
        ...
        {task.description && <p className="text-sm text-zinc-400 whitespace-pre-wrap mb-3">{task.description}</p>}

        <div className="grid grid-cols-2 gap-2 text-xs mb-3">
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-2">
            <span className="text-zinc-500 block mb-1">Responsável</span>
            <select value={task.assigned_to || ''} onChange={e => reassign(e.target.value)} className="w-full bg-transparent text-zinc-200 outline-none">
              <option value="">Sem dono</option>
              {users.map(u => <option key={u.id} value={u.id}>{u.name || u.email}</option>)}
            </select>
          </div>
          <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-2">
            ...
            <span className="text-zinc-200">{PRIO[task.priority].label}</span>
          </div>
        </div>
        {task.due_at && <p className="text-xs text-zinc-500 mb-3 inline-flex items-center gap-1"><Calendar className="w-3 h-3" /> Prazo: {fmt(task.due_at)}</p>}

        {/* Mover */}
        <div className="flex gap-2 mb-4">
          {(['a_fazer', 'fazendo', 'feito'] as const).map(s => (
            <button key={s} onClick={() => onMove(task.id, s)} className={`flex-1 text-[11px] py-1.5 rounded-md border ${task.status === s ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-zinc-950 border-zinc-800 text-zinc-400 hover:text-zinc-200'}`}>
        ...
        <div className="space-y-2 mb-3">
          {(task.updates || []).length === 0 && <p className="text-[11px] text-zinc-600">Sem atualizações ainda.</p>}
          {(task.updates || []).map(u => (
