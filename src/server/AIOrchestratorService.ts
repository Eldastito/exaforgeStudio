...
// { "name": "nome curto", "message": "Olá {nome}! ...mensagem pronta...", "segment": { "inactiveDays": 60 } }`;
    ...
    _rules.push(`IMAGENS: quando a mensagem trouxer a análise de uma imagem (bloco "[Imagem ...]") — se for um DOCUMENTO (comprovante de PIX/pagamento, nota fiscal, recibo, receita/prescrição, boleto): confirme com o cliente o que você leu (ex.: valor, data, nome) e DÊ O PRÓXIMO PASSO coerente (ex.: confirmar o pagamento/pedido). Se for uma FOTO comum (pessoa, animal, objeto, lugar) sem texto relevante: NÃO descreva tecnicamente — pergunte de forma simpática o que o cliente gostaria que você identificasse ou em que pode ajudar com a imagem. Baseie-se SÓ no que a análise trouxe; nunca invente o que não foi identificado.`);
