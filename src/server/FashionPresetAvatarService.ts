...
export class FashionPresetAvatarService {
  ...
  static list(orgId: string, opts: { activeOnly?: boolean } = {}): any[] {
    ...
    return db.prepare(
      `SELECT id, label, body_type, image_url, active, position, created_at
  ...
  static create(orgId: string, input: { label?: string; bodyType?: string; imageUrl: string }): { ok: boolean; id?: string; error?: string } {
    ...
    const id = uuidv4();
    db.prepare(`INSERT INTO fashion_preset_avatars (id, organization_id, label, body_type, image_url, active, position) VALUES (?, ?, ?, ?, ?, 1, ?)`)
      .run(id, orgId, String(input.label || "Modelo").slice(0, 60), bodyType, imageUrl, pos);
    return { ok: true, id };
  }

  static update(orgId: string, id: string, patch: { label?: string; bodyType?: string; active?: boolean; position?: number }): boolean {
