 ...
 *
 * Sempre publica na vitrine (storefront_visible=1, já é o DEFAULT da coluna)
 * — decisão explícita do produto para o cadastro por foto no WhatsApp: o
 * lojista já revisou preço/margem/quantidade na própria conversa antes de
 * chegar aqui, não existe um segundo "publicar" separado.
 */
export class InventoryIntakeService {
  ...
  static commitProductFromScan(orgId: string, params: {
    ...
    salePrice: number; quantity: number; imageUrl: string;
  }): string {
    const id = uuidv4();
    db.prepare(
      `INSERT INTO products_services (id, organization_id, type, name, description, price, stock_control_enabled, category, slug, storefront_visible)
       VALUES (?, ?, 'product', ?, ?, ?, 1, ?, ?, 1)`
    ).run(id, orgId, params.name.trim(), params.description || "", params.salePrice, params.category ? params.category.trim().slice(0, 80) : null, uniqueProductSlug(orgId, params.name));
  ...
  static commitInvoiceItemCreate(orgId: string, params: {
    name: string; category?: string | null; salePrice: number; quantity: number; unitCost: number;
    ...
    db.prepare(
      `INSERT INTO products_services (id, organization_id, type, name, description, price, stock_control_enabled, category, slug, storefront_visible)
       VALUES (?, ?, 'product', ?, '', ?, 1, ?, ?, 1)`
    ).run(productId, orgId, params.name.trim(), params.salePrice, params.category ? params.category.trim().slice(0, 80) : null, uniqueProductSlug(orgId, params.name));
