 ...
 */
/**
 * OCR/RAG — dica de "vendedores cadastrados desta loja" para a IA casar o nome
 * MANUSCRITO com o cadastro (ideia do lojista): dá ao modelo a lista de
 * candidatos reais da loja, então ele sugere o nome mais parecido em vez de
 * chutar a grafia. Vazio quando não há roster (a IA lê como antes).
 */
export function sellerRosterHint(names: Array<string | null | undefined>): string {
...
"cota": <número, a COTA DIÁRIA da loja escrita na folha (ex.: "cota = 3.800" → 3800), ou null>,
 ...
 */
export async function extractSellerSalesFromImage(base64: string, mimetype = "image/jpeg", sellerNames: Array<string | null | undefined> = []): Promise<string> {
